...
def register_quality_callbacks(app):
        ...
        try:
            ...
            if n_clicks is None:
                ...
                return (
                    create_empty_figure(),
                    create_empty_figure(),
                    "Data is available! Click '🎯 Assess Signal Quality' to start analysis.",
                    "",
                    "",
                    "",
                    None,
            ...
            logger.info(
                f"Button clicked {n_clicks} times, starting quality assessment..."
            )
            ...
            main_plot = create_main_quality_plot(
                time_axis,
                windowed_data,
                quality_results,
                signal_type,
            ...
            metrics_plot = create_quality_metrics_plot(
                windowed_data, quality_results, quality_metrics, sampling_freq
            )

            # Create assessment results display
            assessment_results = create_quality_assessment_results(
                quality_results, signal_type, quality_metrics, sampling_freq
            )

            # Create issues and recommendations
            issues_recommendations = create_quality_issues_recommendations(
                quality_results, quality_metrics, snr_threshold, artifact_threshold
            )

            # Create detailed analysis
            detailed_analysis = create_quality_detailed_analysis(
                quality_results, quality_metrics
            )

            # Create quality score dashboard
            score_dashboard = create_quality_score_dashboard(
                quality_results, quality_metrics
            )
    ...
    @app.callback(
        [Output("quality-start-time", "value"), Output("quality-end-time", "value")],
        [
            Input("quality-btn-nudge-m10", "n_clicks"),
            Input("quality-btn-nudge-m1", "n_clicks"),
            Input("quality-btn-nudge-p1", "n_clicks"),
        ...
        [State("quality-start-time", "value"), State("quality-end-time", "value")],
    )
    def update_quality_time_inputs(
...
def assess_signal_quality(
    ...
    try:
        ...
        if "artifacts" in quality_metrics:
            quality_results["artifacts"] = detect_artifacts(
                signal_data, sampling_freq, artifact_threshold
            )

        # Baseline Wander
        if "baseline_wander" in quality_metrics:
            quality_results["baseline_wander"] = assess_baseline_wander(
                signal_data, sampling_freq
            )

        # Motion Artifacts
        if "motion_artifacts" in quality_metrics:
            quality_results["motion_artifacts"] = detect_motion_artifacts(
                signal_data, sampling_freq
            )
        ...
        if "stability" in quality_metrics:
            quality_results["stability"] = assess_signal_stability(
                signal_data, sampling_freq
            )

        # Frequency Content
        if "frequency_content" in quality_metrics:
            quality_results["frequency_content"] = assess_frequency_content(
                signal_data, sampling_freq
            )

        # Peak Detection Quality
        if "peak_quality" in quality_metrics:
            quality_results["peak_quality"] = assess_peak_quality(
                signal_data, sampling_freq
            )
        ...
        quality_results["overall_score"] = calculate_overall_quality_score(
            quality_results
        )
...
def calculate_snr(signal_data, sampling_freq):
    ...
    try:
        ...
        return {
            ...
            "quality": (
                "excellent" if snr_db > 20 else "good" if snr_db > 10 else "poor"
            ),
        }
...
def detect_artifacts(signal_data, sampling_freq, threshold):
    ...
    try:
        ...
        artifact_severity = (
            np.mean(np.abs(signal_data[artifacts] - mean_val)) / std_val
            if len(artifacts) > 0
            else 0
        )
...
def assess_baseline_wander(signal_data, sampling_freq):
    ...
    try:
        ...
        return {
            ...
            "quality": (
                "excellent"
                if wander_percentage < 5
                else "good" if wander_percentage < 15 else "poor"
            ),
        }
    ...
    try:
        ...
        return {
            ...
            "quality": (
                ...
                else "good" if motion_percentage < 10 else "poor"
            ),
        }
...
def assess_signal_amplitude(signal_data):
    ...
    try:
        ...
        amplitude_cv = np.std(signal_data) / np.mean(
            np.abs(signal_data)
        ...
        return {
            ...
            "quality": (
                "excellent"
                if amplitude_cv < 0.3
                else "good" if amplitude_cv < 0.6 else "poor"
            ),
        }
...
def assess_signal_stability(signal_data, sampling_freq):
    ...
    try:
        ...
        if num_segments > 1:
            ...
            stability_cv = (
                np.std(segment_means) / np.mean(segment_means)
                if np.mean(segment_means) > 0
                else 0
            )
        else:
            stability_cv = 0
        ...
        return {
            ...
            "quality": (
                "excellent"
                if stability_cv < 0.1
                else "good" if stability_cv < 0.3 else "poor"
            ),
        }
...
def assess_peak_quality(signal_data, sampling_freq):
    ...
    try:
        ...
        if len(peaks) > 1:
            ...
            height_cv = (
                np.std(peak_heights) / np.mean(peak_heights)
                if np.mean(peak_heights) > 0
                else 0
            )
            ...
            return {
                ...
                "quality": (
                    "excellent"
                    if interval_cv < 0.1 and height_cv < 0.2
                    else "good" if interval_cv < 0.3 and height_cv < 0.5 else "poor"
                ),
            }
        else:
            return {"peak_count": len(peaks), "quality": "poor"}
...
def assess_signal_continuity(signal_data):
    ...
    try:
        ...
        continuity_score = (
            100 - (nan_count + inf_count + len(gaps)) / len(signal_data) * 100
        )
        ...
        return {
            ...
            "quality": (
                ...
                else "good" if continuity_score > 80 else "poor"
            ),
        }
...
def detect_outliers(signal_data):
    ...
    try:
        ...
        outliers = np.where((signal_data < lower_bound) | (signal_data > upper_bound))[
            0
        ]
        ...
        return {
            ...
            "quality": (
                ...
                else "good" if outlier_percentage < 5 else "poor"
            ),
        }
...
def calculate_overall_quality_score(quality_results):
    ...
    try:
        ...
        return {"score": overall_score, "quality": overall_quality}
...
def create_main_quality_plot(
    time_axis, signal_data, quality_results, signal_type, quality_metrics, sampling_freq
):
    ...
    try:
        ...
        if (
            ...
            if len(artifacts) > 0:
                ...
                artifact_values = [
                    signal_data[i] for i in artifacts if i < len(time_axis)
                ]
        ...
        if "overall_score" in quality_results:
            ...
            metrics = [
                m
                for m in metrics
                if m != "overall_score" and "error" not in quality_results.get(m, {})
            ]
            ...
            if metrics:
                ...
                fig.add_trace(
                    go.Bar(
                        x=metrics, y=scores, name="Quality Scores", marker_color="green"
                    ),
                    row=2,
                    col=1,
                )
...
def create_quality_metrics_plot(
    signal_data, quality_results, quality_metrics, sampling_freq
):
    """Create quality metrics visualization."""
    try:
        if "error" in quality_results:
        ...
        fig = make_subplots(
            ...
            cols=1,
            subplot_titles=[
                f"{metric.replace('_', ' ').title()} Analysis"
                ...
                if metric == "snr":
                    ...
                    fig.add_trace(
                        go.Scatter(
                            x=[0, 1],
                            y=[
                                quality_results[metric].get("snr_db", 0),
                                quality_results[metric].get("snr_db", 0),
                            ],
                            mode="lines+markers",
                            name="SNR",
        ...
        fig.update_layout(
            title="Quality Metrics Analysis", height=300 * num_metrics, showlegend=False
        )
...
def create_quality_assessment_results(
    quality_results, signal_type, quality_metrics, sampling_freq
):
    ...
    try:
        ...
        if "overall_score" in quality_results:
            ...
            sections.append(
                html.Div(
                    [
                        html.H5("🎯 Overall Quality Assessment"),
                        html.Div(
                            [
                                html.H6(f"Score: {overall.get('score', 0):.1f}/100"),
                                html.H6(
                                    f"Quality: {overall.get('quality', 'Unknown').title()}"
                                ),
                            ],
        ...
        return html.Div(
            [html.H5("Error"), html.P(f"Failed to create assessment results: {str(e)}")]
        )


def create_quality_issues_recommendations(
    quality_results, quality_metrics, snr_threshold, artifact_threshold
):
        ...
        if "snr" in quality_results and "snr_db" in quality_results["snr"]:
            ...
            if snr_db < snr_threshold:
                ...
                recommendations.append(
                    "Check sensor placement and reduce environmental noise"
                )

        # Check artifacts
        if (
            "artifacts" in quality_results
            and "artifact_percentage" in quality_results["artifacts"]
        ):
            artifact_pct = quality_results["artifacts"]["artifact_percentage"]
            if artifact_pct > 5:
                issues.append("High artifact content")
                recommendations.append(
                    "Check for movement artifacts and improve signal acquisition"
                )

        # Check baseline wander
        if (
            "baseline_wander" in quality_results
            and "wander_percentage" in quality_results["baseline_wander"]
        ):
            wander_pct = quality_results["baseline_wander"]["wander_percentage"]
            if wander_pct > 15:
                issues.append("Significant baseline wander")
                recommendations.append(
                    "Apply high-pass filtering or improve sensor stability"
                )
...
def create_quality_detailed_analysis(quality_results, quality_metrics):
    ...
    try:
        ...
        return html.Div(
            [html.H6("Error"), html.P(f"Failed to create detailed analysis: {str(e)}")]
        )
    ...
    try:
        ...
        score_cards.append(
            html.Div(
                [
                    dbc.Badge(
                        quality.title(), color=quality_color, className="fs-4 p-3"
                    ),
                    html.P(
                        "Quality Level", className="text-center text-muted mb-0 mt-2"
                    ),
                ],
                className="col-md-6",
            )
        ...
        return html.Div(
            [html.H6("Error"), html.P(f"Failed to create quality dashboard: {str(e)}")]
        )
