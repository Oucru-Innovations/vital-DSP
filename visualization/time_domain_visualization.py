...
class TrendAnalysisVisualizer:
    ...
    def plot_trend(self, original_signal, trend, title="Trend Analysis"):
        ...
        # Original signal
        fig.add_trace(go.Scatter(x=list(range(len(original_signal))), y=original_signal,
                                 mode='lines', name='Original Signal'))

        # Trend line
        fig.add_trace(go.Scatter(x=list(range(len(trend))), y=trend,
    ...
    def plot_moving_average(self, moving_avg, window_size, title="Moving Average"):
        self.plot_trend(self.trend_analysis.signal, moving_avg, f"{title} (Window Size: {window_size})")

    def plot_weighted_moving_average(self, weighted_avg, title="Weighted Moving Average"):
        self.plot_trend(self.trend_analysis.signal, weighted_avg, title)

    def plot_exponential_smoothing(self, exp_smoothed, alpha, title="Exponential Smoothing"):
        self.plot_trend(self.trend_analysis.signal, exp_smoothed, f"{title} (Alpha: {alpha})")
    ...
    def plot_polynomial_trend(self, polynomial_trend, degree, title="Polynomial Trend"):
        self.plot_trend(self.trend_analysis.signal, polynomial_trend, f"{title} (Degree: {degree})")
...
class SignalSegmentationVisualizer:
    ...
    def plot_segments(self, segments, title="Signal Segmentation"):
        ...
        for i, segment in enumerate(segments):
            fig.add_trace(go.Scatter(x=list(range(len(segment))), y=segment,
                                     mode='lines', name=f'Segment {i+1}'))
...
class SignalPowerAnalysisVisualizer:
    ...
    def plot_signal_with_power(self, signal, power, title="Signal with Power"):
        ...
        # Original signal
        fig.add_trace(go.Scatter(x=list(range(len(signal))), y=signal,
                                 mode='lines', name='Original Signal'))

        # Signal power (as a line or area under the curve)
        fig.add_trace(go.Scatter(x=list(range(len(signal))), y=power,
...
class SignalChangeDetectionVisualizer:
    ...
    def plot_changes(self, original_signal, change_points, title="Signal Change Detection"):
        fig = go.Figure()
        ...
        # Mark change points
        fig.add_trace(go.Scatter(x=change_points, y=[original_signal[i] for i in change_points],
...
class PeakDetectionVisualizer:
    ...
    def plot_peaks(self, original_signal, peaks, title="Peak Detection"):
        ...
        # Mark peaks
        fig.add_trace(go.Scatter(x=peaks, y=[original_signal[i] for i in peaks],
                                 mode='markers', name='Peaks', marker=dict(color='orange', size=10)))
...
class EnvelopeDetectionVisualizer:
    ...
    def plot_envelope(self, original_signal, envelope, title="Envelope Detection"):
        ...
        # Envelope
        fig.add_trace(go.Scatter(x=list(range(len(envelope))), y=envelope,
                                 mode='lines', name='Envelope', line=dict(dash='dash')))
...
class CrossSignalAnalysisVisualizer:
    ...
    def plot_cross_signal(self, signal1, signal2, title="Cross Signal Analysis"):
        ...
        # Signal 1
        fig.add_trace(go.Scatter(x=list(range(len(signal1))), y=signal1,
                                 mode='lines', name='Signal 1'))

        # Signal 2
        fig.add_trace(go.Scatter(x=list(range(len(signal2))), y=signal2,
                                 mode='lines', name='Signal 2'))
    ...
    def plot_coherence(self, freqs, coherence, title="Coherence Between Signals"):
        ...
        # Coherence
        fig.add_trace(go.Scatter(x=freqs, y=coherence,
    ...
    def plot_cross_correlation(self, lags, cross_corr, title="Cross-Correlation Between Signals"):
        fig = go.Figure()

        # Cross-Correlation
        fig.add_trace(go.Scatter(x=lags, y=cross_corr,
