...
class PeakDetection:
    ...
    def detect_peaks(self):
        ...
        if self.method == 'threshold':
            ...
            raise ValueError("Invalid method selected. Choose from the provided methods.")
    ...
    def _savgol_based_detection(self):
        ...
        smoothed_signal = SignalFiltering.savgol_filter(self.signal, window_length=window_length, polyorder=polyorder)
    ...
    def _abp_systolic_peak_detection(self):
        smoothed_signal = SignalFiltering.savgol_filter(self.signal, window_length=11, polyorder=2)
        return find_peaks(smoothed_signal, distance=60)

    def _abp_diastolic_peak_detection(self):
        inverted_signal = -self.signal
        smoothed_signal = SignalFiltering.savgol_filter(inverted_signal, window_length=11, polyorder=2)
        return find_peaks(smoothed_signal, distance=60)
