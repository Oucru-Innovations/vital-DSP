...
def setup_logging(debug_mode: bool = False):
    ...
    if debug_mode:
        ...
        logging.getLogger(
            "src.vitalDSP_webapp.services.data.enhanced_data_service"
        ).setLevel(logging.DEBUG)
        logging.getLogger(
            "vitalDSP_webapp.services.data.enhanced_data_service"
        ).setLevel(logging.DEBUG)
    else:
...
def run_webapp(debug_mode: bool = False, port: int = None, host: str = "0.0.0.0"):
    ...
    uvicorn.run(
        fastapi_app, host=host, port=port, log_level="info"  # Use consistent log level
    )
