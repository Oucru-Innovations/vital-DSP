...
def register_advanced_callbacks(app):
    ...
    def advanced_analysis_callback(
        ...
        # ML Parameters - SVM
        svm_enable, svm_kernel, svm_c, svm_gamma,
        # ML Parameters - Random Forest
        rf_enable, rf_n_estimators, rf_max_depth, rf_min_samples_split,
        # ML Parameters - Neural Network
        nn_enable, nn_hidden_layers, nn_activation, nn_learning_rate,
        # DL Parameters - LSTM
        lstm_enable, lstm_units, lstm_layers, lstm_dropout, lstm_lr,
        # DL Parameters - CNN
        cnn_enable, cnn_filters, cnn_kernel, cnn_pool, cnn_dropout,
        # DL Parameters - Transformer
        transformer_enable, transformer_heads, transformer_layers, transformer_dim,
    ):
        ...
        if n_clicks:
            try:
                ...
                logger.info(f"Data shape: {df.shape if hasattr(df, 'shape') else 'N/A'}, columns: {list(df.columns) if hasattr(df, 'columns') else 'N/A'}")

                # Handle case where df might be a dict instead of DataFrame
                if isinstance(df, dict):
                    logger.warning("get_data returned dict instead of DataFrame, extracting 'data' key")
                    df = df.get("data", None)
                ...
                if not isinstance(df, pd.DataFrame):
                    ...
                    error_results = html.Div(
                        [
                            ...
                            html.P(f"Expected DataFrame but got {type(df).__name__}. Please re-upload your data."),
                        ]
                    )
                    return (
                        error_fig,
                ...
                end_time = start_sample / sampling_freq + (end_sample - start_sample) / sampling_freq
                ...
                logger.info(f"  Requested: start={start_position}%, duration={duration}s")
                logger.info(f"  Total data available: {total_duration:.2f}s ({len(df)} samples)")
                logger.info(f"  Start time: {start_time_actual:.2f}s")
                logger.info(f"  Desired end time: {desired_end_time:.2f}s")
                logger.info(f"  Sample range: {start_sample} to {end_sample} ({actual_window_size} samples)")
                ...
                if signal_source == "filtered" and filtered_signal_data:
                    ...
                    logger.info(f"Using filtered signal. Full length: {len(filtered_signal_full)}, extracting indices {start_sample} to {end_sample}")
                    ...
                    if len(filtered_signal_full) >= end_sample:
                        ...
                        logger.info(f"Extracted filtered signal window: {len(signal_data)} samples ({len(signal_data)/sampling_freq:.2f}s)")
                    else:
                        logger.warning(f"Filtered signal too short ({len(filtered_signal_full)} < {end_sample}), using original signal")
                    ...
                    logger.info(f"Using original signal. Windowed DataFrame: {start_sample} to {end_sample} from {len(df)} samples")
                    windowed_df = df.iloc[start_sample:end_sample].copy()
                    signal_col = column_mapping.get("signal", "signal")
                    if signal_col and signal_col in windowed_df.columns:
                        signal_data = windowed_df[signal_col].values
                ...
                logger.info(f"Final signal data length: {len(signal_data)} samples ({len(signal_data)/sampling_freq:.2f} seconds)")
                logger.info(f"Signal data range: {np.min(signal_data):.2f} to {np.max(signal_data):.2f}")
                ...
                logger.info(f"Time axis range: {time_axis[0]:.2f}s to {time_axis[-1]:.2f}s")
                ...
                ml_params = {
                    "svm": {"enable": svm_enable, "kernel": svm_kernel or "rbf", "C": svm_c or 1.0, "gamma": svm_gamma or "scale"},
                    "random_forest": {"enable": rf_enable, "n_estimators": rf_n_estimators or 100, 
                                     "max_depth": rf_max_depth or 10, "min_samples_split": rf_min_samples_split or 2},
                    "neural_network": {"enable": nn_enable, "hidden_layers": nn_hidden_layers or "100,50",
                                      "activation": nn_activation or "relu", "learning_rate": nn_learning_rate or 0.001},
                }
                
                # Prepare DL model parameters
                dl_params = {
                    "lstm": {"enable": lstm_enable, "units": lstm_units or 128, "layers": lstm_layers or 2, "dropout": lstm_dropout or 0.2,
                            "learning_rate": lstm_lr or 0.001},
                    "cnn": {"enable": cnn_enable, "filters": cnn_filters or "64,128,256", "kernel": cnn_kernel or 3,
                           "pool": cnn_pool or 2, "dropout": cnn_dropout or 0.3},
                    "transformer": {"enable": transformer_enable, "heads": transformer_heads or 8,
                                   "layers": transformer_layers or 4, "dim": transformer_dim or 512},
                }
                logger.info(f"ML params prepared: svm_enable={svm_enable}, rf_enable={rf_enable}")
                logger.info(f"DL params prepared: lstm_enable={lstm_enable}, cnn_enable={cnn_enable}")
                ...
                detailed_report = create_detailed_analysis_report(
                    analysis_results, analysis_categories, signal_type, time_axis, signal_data, sampling_freq
                )
                ...
                error_results = html.Div(
                    [
                        ...
                        html.Pre(traceback.format_exc(), style={"fontSize": "10px", "whiteSpace": "pre-wrap"}),
                    ]
                )
                return (
                    error_fig,
    ...
    try:
        ...
        if "ml_analysis" in analysis_categories and ml_options:
            analysis_results["ml_results"] = perform_ml_analysis(
                signal_data, sampling_freq, ml_options, cv_folds, random_state, ml_params
            )
        ...
        if "dimensionality_reduction" in analysis_categories:
            analysis_results["dimensionality_reduction"] = perform_dimensionality_reduction_analysis(
                signal_data, sampling_freq
            )
...
def perform_ml_analysis(signal_data, sampling_freq, ml_options, cv_folds, random_state, ml_params=None):
    ...
    try:
        ...
        if "svm" in ml_options:
            ...
            ml_results["svm"] = train_svm_model(features, cv_folds, random_state, svm_params)
...
def perform_deep_learning_analysis(signal_data, sampling_freq, deep_learning_options, dl_params=None):
    ...
    try:
        ...
        if "transformer" in deep_learning_options:
            ...
            dl_results["transformer"] = train_transformer_model(dl_data, transformer_params)
...
def perform_anomaly_detection(signal_data, sampling_freq):
    ...
    try:
        ...
        try:
            anomalies_z_score = anomaly_detector.detect_anomalies(method="z_score", threshold=3.0)
            results["z_score_anomalies"] = len(anomalies_z_score)
            results["z_score_rate"] = len(anomalies_z_score) / len(signal_data) * 100 if len(signal_data) > 0 else 0
        except Exception as e:
        ...
        try:
            ...
            results["moving_average_rate"] = len(anomalies_ma) / len(signal_data) * 100 if len(signal_data) > 0 else 0
        except Exception as e:
        ...
        try:
            ...
            if n_neighbors >= 2:
                anomalies_lof = anomaly_detector.detect_anomalies(method="lof", n_neighbors=n_neighbors)
                results["lof_anomalies"] = len(anomalies_lof)
                results["lof_rate"] = len(anomalies_lof) / len(signal_data) * 100 if len(signal_data) > 0 else 0
            else:
        ...
        # FFT based detection
        try:
            anomalies_fft = anomaly_detector.detect_anomalies(method="fft", threshold=1.5)
            results["fft_anomalies"] = len(anomalies_fft)
            results["fft_rate"] = len(anomalies_fft) / len(signal_data) * 100 if len(signal_data) > 0 else 0
        except Exception as e:
        ...
        if "moving_average_anomalies" in results and results.get("moving_average_anomalies", 0) > 0:
        ...
        confirmed_anomalies = [idx for idx, votes in anomaly_votes.items() if votes >= min_votes]
        ...
        if len(confirmed_anomalies) > max_anomalies:
            ...
            logger.warning(f"Anomaly detection found {len(confirmed_anomalies)} anomalies ({len(confirmed_anomalies)/len(signal_data)*100:.1f}%), capped at {max_anomalies}")
        
        results["total_anomalies"] = len(confirmed_anomalies)
        results["overall_anomaly_rate"] = len(confirmed_anomalies) / len(signal_data) * 100 if len(signal_data) > 0 else 0
        ...
        logger.info(f"Anomaly detection complete: {results['total_anomalies']} anomalies found ({results['overall_anomaly_rate']:.2f}%)")
...
def perform_classification_analysis(signal_data, sampling_freq):
    ...
    try:
        ...
        results["total_classes"] = len([k for k in results.keys() if k != "method" and "error" not in str(results[k])])
...
def perform_regression_analysis(signal_data, sampling_freq):
    ...
    try:
            ...
            if peak_count > 1:
                ...
                if len(intervals) > 2:
                    ...
                    results["hrv_regression"] = {
                        ...
                        "trend_direction": "Increasing" if interval_trend > 0 else "Decreasing",
                        "peak_count": peak_count,
                        "method": "HRV Interval Regression"
                    }
                else:
                    results["hrv_regression"] = {"error": "Insufficient intervals for regression"}
            else:
                results["hrv_regression"] = {"error": "Insufficient peaks for HRV analysis"}
        except Exception as e:
        ...
        results["total_regressions"] = len([k for k in results.keys() if k != "method" and "error" not in str(results[k])])
    ...
    try:
        ...
        try:
            ...
            if len(segments) > 1:
                ...
                results["segment_clustering"] = {"error": "Insufficient segments for clustering"}
        except ImportError:
            ...
            if peak_count > 3:
                ...
                if len(peaks) > 3:
                    ...
                    if len(peak_features) > 3:
                        ...
                        high_amplitude_cluster = peak_features[peak_features[:, 0] > amplitude_threshold]
                        low_amplitude_cluster = peak_features[peak_features[:, 0] <= amplitude_threshold]
                        ...
                        results["peak_clustering"] = {"error": "Insufficient peak features"}
                else:
                    results["peak_clustering"] = {"error": "Insufficient peaks"}
            else:
                results["peak_clustering"] = {"error": "Insufficient peaks for clustering"}
        except Exception as e:
        ...
        results["total_clusters"] = len([k for k in results.keys() if k != "method" and "error" not in str(results[k])])
...
def perform_dimensionality_reduction_analysis(signal_data, sampling_freq):
    ...
    try:
        ...
        try:
            ...
            for i in range(min(5, len(feature_names))):
                ...
                top_features.append({
                    ...
                    "importance": feature_variances[idx] / np.sum(feature_variances)
                })
            
            results["pca_analysis"] = {
                "top_features": top_features,
                "total_features": len(feature_names),
                "explained_variance_ratio": (feature_variances / np.sum(feature_variances)).tolist(),
        ...
        try:
            ...
            normalized_features = np.abs(feature_values) / np.max(np.abs(feature_values))
            feature_importance = []
            
            for i, (name, value, norm_value) in enumerate(zip(feature_names, feature_values, normalized_features)):
            ...
            # Sort by importance
            feature_importance.sort(key=lambda x: x["normalized_importance"], reverse=True)
        ...
        results["total_methods"] = len([k for k in results.keys() if k != "method" and "error" not in str(results[k])])
...
def perform_forecasting_analysis(signal_data, sampling_freq):
    ...
    try:
        ...
        try:
            ...
            forecast_duration = min(5.0, len(signal_data) / sampling_freq * 0.1)  # 5 seconds max
            forecast_length = int(forecast_duration * sampling_freq)
            forecast_length = max(10, min(forecast_length, len(signal_data) // 5))  # At least 10, at most 20% of signal
            
            future_time_points = np.arange(len(signal_data), len(signal_data) + forecast_length)
        ...
        try:
            ...
            if len(signal_data) > 100:
                ...
                heart_rate = temporal_features.get("heart_rate", 0)
                if heart_rate > 0:
                    period_samples = int(sampling_freq * 60 / heart_rate)  # One heart beat period
                    window_size = max(10, min(period_samples * 2, len(signal_data) // 10))
                else:
            ...
            if len(moving_avg) > 5:
                ma_trend = np.polyfit(ma_time[-min(20, len(moving_avg)):], 
                                     moving_avg[-min(20, len(moving_avg)):], 1)
            else:
                ma_trend = [0, moving_avg[-1] if len(moving_avg) > 0 else np.mean(signal_data)]
            
            # Forecast with trend
            forecast_length = max(10, min(len(signal_data) // 10, int(5 * sampling_freq)))
            future_ma_indices = np.arange(len(moving_avg), len(moving_avg) + forecast_length)
            ...
            results["moving_average_forecast"] = {
                ...
                "last_ma_value": moving_avg[-1] if len(moving_avg) > 0 else np.mean(signal_data),
            ...
            if heart_rate > 0:
                ...
                if window_size_samples < len(signal_data) // 4:
                    ...
                    for i in range(num_windows):
                        ...
                        window_end = min(window_start + window_size_samples, len(signal_data))
                        window_data = signal_data[window_start:window_end]
                        try:
                            window_peaks, _ = signal.find_peaks(
                                window_data, 
                                distance=int(sampling_freq / heart_rate * 0.6),  # At least 60% of expected interval
                                height=np.percentile(window_data, 75)
                            )
                            if len(window_peaks) > 1:
                                intervals = np.diff(window_peaks) / sampling_freq
                                hr_window = 60.0 / np.mean(intervals) if np.mean(intervals) > 0 else heart_rate
                    ...
                    if len(hr_trend) >= 2:
                        ...
                        future_hr_times = np.arange(len(hr_trend), len(hr_trend) + (forecast_length // window_size_samples) + 1)
                        ...
                        if len(forecast_hr_trend) > 1:
                            forecast_hr = np.interp(
                                ...
                                np.linspace(0, forecast_length-1, len(forecast_hr_trend)),
                                forecast_hr_trend
                            )
                        else:
                            forecast_hr = np.full(forecast_length, forecast_hr_trend[0] if len(forecast_hr_trend) > 0 else heart_rate)
        ...
        try:
            ...
            # Forecast amplitude
            forecast_length = max(10, len(signal_data) // 10)
            future_time_points = np.arange(len(recent_signal), len(recent_signal) + forecast_length)
        ...
        results["total_forecasts"] = len([k for k in results.keys() if k != "method" and "error" not in str(results[k])])
...
def extract_temporal_features(signal_data, sampling_freq):
    ...
    try:
        ...
        peaks = wm.critical_points.get("r_peaks", wm.r_peaks) if hasattr(wm, 'critical_points') else wm.r_peaks
        ...
        if peaks is not None and len(peaks) > 1:
            ...
            if mean_interval > 0:
                ...
                if heart_rate > 200:
                    logger.warning(f"vitalDSP calculated HR ({heart_rate:.1f} bpm) exceeds limit. Found {len(peaks)} peaks - likely over-detection.")
                    logger.warning(f"Falling back to basic peak detection...")
                    use_basic_detection = True
                elif heart_rate < 30:
                    logger.warning(f"Calculated HR ({heart_rate:.1f} bpm) below typical physiological limit (<30 bpm).")
            else:
                heart_rate = 0

            if not use_basic_detection:
                logger.info(f"vitalDSP peak detection: found {len(peaks)} peaks, HR={heart_rate:.1f} bpm")
            return {
        ...
        if use_basic_detection or peaks is None or len(peaks) <= 1:
            ...
            logger.info("vitalDSP peak detection returned no peaks, using basic peak detection")
            ...
            peaks_basic, _ = signal.find_peaks(signal_data, height=threshold, distance=min_peak_distance)
            ...
            if len(peaks_basic) > 1:
                ...
                logger.info(f"Basic peak detection found {len(peaks_basic)} peaks, HR={heart_rate:.1f} bpm")
                
                # If HR is unrealistic, try more aggressive filtering
                if heart_rate > 250:
                    logger.warning(f"HR {heart_rate:.1f} bpm too high, applying more aggressive peak detection")
                    ...
                    if len(peaks_basic) > 1:
                        ...
                        mean_interval = np.mean(intervals)
                        heart_rate = 60 / mean_interval if mean_interval > 0 else 0
                        logger.info(f"After aggressive filtering: {len(peaks_basic)} peaks, HR={heart_rate:.1f} bpm")
        ...
        logger.warning("vitalDSP not available for peak detection, using scipy fallback")
        # Fallback to scipy peak detection
        threshold = np.mean(signal_data) + 1.5 * np.std(signal_data)
        min_peak_distance = int(0.3 * sampling_freq)
        peaks, _ = signal.find_peaks(signal_data, height=threshold, distance=min_peak_distance)
...
def train_lstm_model(data, lstm_params=None):
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"LSTM training failed: {train_error}. Returning placeholder result.")
            mse = 0.0
...
def create_advanced_visualizations(analysis_results, time_axis, signal_data, sampling_freq):
    ...
    try:
        ...
        fig.add_trace(
            ...
            row=row1_col1[0], col=row1_col1[1]
        )
        
        # 2. Feature Analysis overlay - normalized feature scores as envelope
        if "features" in analysis_results and "error" not in str(analysis_results.get("features", {})):
            try:
                ...
                if "statistical" in features and "error" not in str(features.get("statistical", {})):
                ...
                if "spectral" in features and "error" not in str(features.get("spectral", {})):
                    spec = features["spectral"]
                    feature_values.append(np.log1p(spec.get('spectral_centroid', 0)) * 0.2)
                    feature_values.append(np.log1p(spec.get('dominant_frequency', 0)) * 0.2)
                
                if "temporal" in features and "error" not in str(features.get("temporal", {})):
                ...
                if feature_values:
                    ...
                    if signal_range > 0:
                        envelope_scaled = np.min(signal_data) + envelope * signal_range * 0.5
        ...
        try:
            if "patterns" in analysis_results and "peaks" in analysis_results["patterns"]:
                peaks_info = analysis_results["patterns"]["peaks"]
                if peaks_info.get("num_peaks", 0) > 0:
                    temporal_features = extract_temporal_features(signal_data, sampling_freq)
                    peaks = temporal_features.get("peaks", [])
                    if len(peaks) > 0 and isinstance(peaks, np.ndarray) or (isinstance(peaks, list) and len(peaks) > 0):
                        ...
                        if len(valid_peaks) > 0:
                            ...
                            fig.add_trace(
                                go.Scatter(
                                    ...
                                    marker=dict(color="red", size=10, symbol="diamond", line=dict(width=1, color="darkred")),
            ...
            if "advanced_processing" in analysis_results:
                ...
                if "emd" in adv and "error" not in str(adv.get("emd", {})):
                    ...
                    if "imfs" in emd and len(emd["imfs"]) > 0:
                        ...
                        high_freq_component = np.sum([imfs[i] for i in range(num_imfs_to_show) if len(imfs[i]) == len(signal_data)], axis=0)
                ...
                # Try wavelet if available
                    ...
                    if "coefficients" in wavelet and len(wavelet["coefficients"]) > 0:
                        ...
                        coeffs = wavelet["coefficients"][0] if isinstance(wavelet["coefficients"], list) else wavelet["coefficients"]
                        if len(coeffs) <= len(time_axis):
                            # Interpolate to match signal length
                            coeffs_interp = np.interp(np.arange(len(signal_data)), 
        ...
        if "classification" in analysis_results and "error" not in str(analysis_results["classification"]):
            class_results = analysis_results["classification"]
            for key, value in class_results.items():
                if isinstance(value, dict) and "error" not in str(value) and "class" in value:
        ...
        if classification_summary:
            ...
            for c in classes:
                if isinstance(analysis_results["classification"][c], dict):
                    conf = analysis_results["classification"][c].get("confidence", 0.5) * 100
                else:
                    conf = 0.5
            ...
            fig.add_trace(
                ...
                row=row1_col2[0], col=row1_col2[1]
            )
        else:
            fig.add_trace(
        ...
        if "anomaly_detection" in analysis_results and "error" not in analysis_results["anomaly_detection"]:
            ...
            if "total_anomalies" in anomaly and anomaly["total_anomalies"] > 0:
                ...
                if std_val > 0:
                    ...
                    fig.add_trace(
                        ...
                        row=row2_col1[0], col=row2_col1[1]
                    )
            ...
            fig.add_trace(
                go.Scatter(x=time_axis, y=signal_data, mode="lines", name="Signal", line=dict(color="blue")),
                row=row2_col1[0], col=row2_col1[1]
            )
        ...
        # Show pattern types
        if "clustering" in analysis_results and "segment_clustering" in analysis_results["clustering"]:
                ...
                for i, label in enumerate(cluster_labels):
                    ...
                    if start_idx < len(time_axis):
                        ...
                        fig.add_trace(
                            ...
                            row=row2_col2[0], col=row2_col2[1]
                        )
        else:
            fig.add_trace(
                go.Scatter(x=time_axis, y=signal_data, mode="lines", name="Signal", line=dict(color="green")),
                row=row2_col2[0], col=row2_col2[1]
            )
        ...
        if "advanced_processing" in analysis_results and "emd" in analysis_results["advanced_processing"]:
                    ...
                    for i in range(min(3, len(imfs))):
                        if i == 0 or i == 1 or i == 2:
                            fig.add_trace(
                                ...
                                row=row3_col1[0], col=row3_col1[1]
                            )
        
        if "advanced_processing" not in analysis_results or "emd" not in analysis_results["advanced_processing"]:
            fig.add_trace(
                go.Scatter(x=time_axis, y=signal_data, mode="lines", name="Signal", line=dict(color="purple")),
                row=row3_col1[0], col=row3_col1[1]
            )
        ...
        if "forecasting" in analysis_results and "linear_forecast" in analysis_results["forecasting"]:
            ...
            if "forecast_values" in fc and "error" not in fc:
                ...
                time_step = time_axis[1] - time_axis[0] if len(time_axis) > 1 else 1 / sampling_freq
                forecast_time = np.arange(time_axis[-1], time_axis[-1] + len(forecast_values) * time_step, time_step)
            ...
            fig.add_trace(
                    ...
                    row=row3_col2[0], col=row3_col2[1]
                )
            ...
            fig.add_trace(
                go.Scatter(x=time_axis, y=signal_data, mode="lines", name="Signal", line=dict(color="cyan")),
                row=row3_col2[0], col=row3_col2[1]
            )

        # Update layout
            fig.update_layout(
            title="Advanced Analysis - Comprehensive Visualizations",
            height=1200,
            showlegend=True,
...
def create_anomaly_detection_plot(signal_data, time_axis, anomaly_results, sampling_freq):
    ...
    try:
        ...
        if "slope" in regression_results:
            ...
            if "intercept" in regression_results:
                ...
                intercept = np.mean(signal_data) - slope * np.mean(np.arange(len(signal_data)))
...
def create_emd_visualization(signal_data, time_axis, emd_results):
    ...
    try:
        ...
        # Original signal
        fig.add_trace(
            go.Scatter(x=time_axis, y=signal_data, mode="lines", name="Original", line=dict(color="blue")),
            row=1, col=1
        )
...
def create_detailed_analysis_report(analysis_results, analysis_categories, signal_type, time_axis, signal_data, sampling_freq):
    ...
    try:
        ...
        if "features" in analysis_results:
            ...
            if "statistical" in features and "error" not in features["statistical"]:
                ...
                feature_table_rows.extend([
                    html.Tr([html.Td(html.Strong("Mean:")), html.Td(f"{stat.get('mean', 0):.6f}")]),
                    html.Tr([html.Td(html.Strong("Std Dev:")), html.Td(f"{stat.get('std', 0):.6f}")]),
                    html.Tr([html.Td(html.Strong("Skewness:")), html.Td(f"{stat.get('skewness', 0):.6f}")]),
                ])
            
            if "temporal" in features and "error" not in features["temporal"]:
                temp = features["temporal"]
                feature_table_rows.extend([
                    html.Tr([html.Td(html.Strong("Peak Count:")), html.Td(f"{temp.get('peak_count', 0)}")]),
                    html.Tr([html.Td(html.Strong("Heart Rate:")), html.Td(f"{temp.get('heart_rate', 0):.2f} bpm")]),
                ])
            
            if feature_table_rows:
                sections.append(html.Table([
                    html.Tbody(feature_table_rows)
        ...
        if "anomaly_detection" in analysis_results and "error" not in analysis_results["anomaly_detection"]:
            ...
            sections.append(html.Div([
                ...
                html.P(html.Strong(f"Detection Method: {anomaly.get('method', 'N/A')}")),
                html.P(f"Total anomalies detected: {total_anomalies} ({anomaly_rate:.2f}% of signal)"),
                html.P(f"Methods used: {', '.join(anomaly.get('methods_used', []))}") if "methods_used" in anomaly else None,
                warning_msg if warning_msg else None,
                dcc.Graph(figure=anomaly_fig, style={"height": "400px"}) if anomaly_fig else None,
            ]))
        ...
        if "classification" in analysis_results and "error" not in str(analysis_results["classification"]):
            ...
            sections.append(html.Div([
                html.H5("3. Classification Results", className="text-success mt-4"),
            ]))
            ...
            for key, value in classification.items():
                if isinstance(value, dict) and "error" not in str(value):
                    if "class" in value:
                        ...
                        class_results.append(html.P(
                            f"• {class_name}: {html.Strong(class_val)} (Confidence: {conf:.1%})",
                            style={"marginBottom": "0.5rem"}
                        ))
                        class_data[class_name] = {"class": class_val, "confidence": conf}
            ...
            if class_results:
                ...
                sections.append(html.P("No classification results available.", className="text-muted"))
        
        # 4. Regression Analysis
        if "regression" in analysis_results and "error" not in analysis_results["regression"]:
            regression = analysis_results["regression"]
            sections.append(html.Div([
                html.H5("4. Regression Analysis", className="text-info mt-4"),
            ]))
            
            if "linear_trend_regression" in regression and "error" not in regression["linear_trend_regression"]:
                ...
                rsq_warning = ""
                if r_squared < 0.01:
                    rsq_warning = html.P(html.Small("⚠️ Note: Very low R-squared indicates weak linear relationship."), className="text-muted mt-1")
                ...
                if regression_fig:
                    sections.append(dcc.Graph(figure=regression_fig, style={"height": "300px"}))
        
        # 5. Clustering Results
        if "clustering" in analysis_results and "error" not in analysis_results["clustering"]:
            clustering = analysis_results["clustering"]
            sections.append(html.Div([
                html.H5("5. Clustering Analysis", className="text-warning mt-4"),
            ]))
            
            if "segment_clustering" in clustering and "error" not in clustering["segment_clustering"]:
                seg = clustering["segment_clustering"]
                sections.append(html.P(f"""
                ...
                if clustering_fig:
                    sections.append(dcc.Graph(figure=clustering_fig, style={"height": "300px"}))
        
        # 6. Forecasting
        if "forecasting" in analysis_results and "error" not in analysis_results["forecasting"]:
            forecasting = analysis_results["forecasting"]
            sections.append(html.Div([
                html.H5("6. Forecasting Analysis", className="text-secondary mt-4"),
            ]))
            
            if "linear_forecast" in forecasting and "error" not in forecasting["linear_forecast"]:
                lf = forecasting["linear_forecast"]
                forecast_duration = lf.get('forecast_length', 0) / sampling_freq if sampling_freq > 0 else 0
                
                sections.append(html.Div([
                    html.P(html.Strong(f"Method: {lf.get('method', 'Linear Forecasting')}")),
                    html.Table([
                        html.Tbody([
                            html.Tr([html.Td(html.Strong("Forecast Length:")), html.Td(f"{lf.get('forecast_length', 0)} samples ({forecast_duration:.2f} seconds)")]),
                            html.Tr([html.Td(html.Strong("Trend Slope:")), html.Td(f"{lf.get('trend_slope', 0):.6f}")]),
                            html.Tr([html.Td(html.Strong("RMSE:")), html.Td(f"{lf.get('rmse', 0):.4f}")]) if "rmse" in lf else None,
                            html.Tr([html.Td(html.Strong("R-squared:")), html.Td(f"{lf.get('r_squared', 0):.4f}")]) if "r_squared" in lf else None,
                            html.Tr([html.Td(html.Strong("Confidence Interval:")), html.Td(f"±{lf.get('confidence_interval', 0):.4f}")]) if "confidence_interval" in lf else None,
                        ])
                    ], className="table table-sm table-bordered mb-3")
                ]))
                
                # Create forecasting visualization
                forecast_fig = create_forecast_plot(signal_data, time_axis, lf, sampling_freq)
                if forecast_fig:
                    sections.append(dcc.Graph(figure=forecast_fig, style={"height": "300px"}))
        ...
        if "advanced_processing" in analysis_results:
            ...
            if "emd" in adv and "error" not in adv["emd"]:
                ...
                sections.append(html.P(html.Strong(f"""
                ...
                if num_imfs > 0:
                    sections.append(html.H6("IMF Analysis Details", className="mt-3 mb-2"))
                    ...
                    for i, imf in enumerate(imfs):
                        if len(imf) == len(time_axis):
                            ...
                            total_energy = np.sum([np.sum(imf_arr**2) for imf_arr in imfs if len(imf_arr) == len(time_axis)])
                            imf_energy_pct = (imf_energy / total_energy * 100) if total_energy > 0 else 0
                            ...
                            imf_details.append(html.Div([
                                html.H6(f"IMF {i+1}", className="mb-1"),
                                html.P(f"Description: {imf_desc}", style={"fontSize": "0.9em", "color": "#666"}),
                                html.Table([
                                    html.Tbody([
                                        html.Tr([html.Td(html.Strong("Mean:")), html.Td(f"{imf_mean:.4f}")]),
                                        html.Tr([html.Td(html.Strong("Std Dev:")), html.Td(f"{imf_std:.4f}")]),
                                        html.Tr([html.Td(html.Strong("Energy:")), html.Td(f"{imf_energy:.2f} ({imf_energy_pct:.1f}% of total)")]),
                                        html.Tr([html.Td(html.Strong("Dominant Frequency:")), html.Td(f"{dominant_freq:.2f} Hz")]),
                                        html.Tr([html.Td(html.Strong("Amplitude Range:")), html.Td(f"{np.min(imf):.2f} to {np.max(imf):.2f}")]),
                                    ])
                                ], className="table table-sm table-bordered mb-3")
                    ...
                    if len(residual) == len(time_axis):
                        sections.append(html.Div([
                            ...
                            html.Table([
                                html.Tbody([
                                    html.Tr([html.Td(html.Strong("Mean:")), html.Td(f"{np.mean(residual):.4f}")]),
                                    html.Tr([html.Td(html.Strong("Std Dev:")), html.Td(f"{np.std(residual):.4f}")]),
                                    html.Tr([html.Td(html.Strong("Range:")), html.Td(f"{np.min(residual):.2f} to {np.max(residual):.2f}")]),
                                ])
                ...
                if emd_fig:
                    sections.append(dcc.Graph(figure=emd_fig, style={"height": "600px"}))
        ...
        sections.append(html.Div([
            ...
            html.Table([
                html.Tbody([
                    html.Tr([html.Td("Signal Length"), html.Td(f"{len(signal_data)} samples")]),
                    html.Tr([html.Td("Duration"), html.Td(f"{len(signal_data)/sampling_freq:.2f} seconds")]),
                    html.Tr([html.Td("Sampling Frequency"), html.Td(f"{sampling_freq:.1f} Hz")]),
                    html.Tr([html.Td("Signal Range"), html.Td(f"{np.min(signal_data):.2f} to {np.max(signal_data):.2f}")]),
                ])
        ...
        return html.Div([
            html.H6("Report Error"),
            html.P(f"Failed to generate report: {str(e)}")
        ])
...
def create_advanced_analysis_summary(analysis_results, signal_type):
    ...
    try:
        ...
        if "anomaly_detection" in analysis_results and "error" not in analysis_results["anomaly_detection"]:
            ...
            sections.append(html.P(f"Total Anomalies: {anomaly.get('total_anomalies', 0)}"))
            sections.append(html.P(f"Anomaly Rate: {anomaly.get('overall_anomaly_rate', 0):.2f}%"))
            if "z_score_anomalies" in anomaly:
                sections.append(html.P(f"Z-score anomalies: {anomaly.get('z_score_anomalies', 0)} ({anomaly.get('z_score_rate', 0):.2f}%)"))
        ...
        if "classification" in analysis_results and "error" not in analysis_results["classification"]:
            ...
            if "signal_quality_classification" in classification:
                ...
                if "class" in sq_class:
                    ...
                    sections.append(html.P(f"  • Class: {sq_class.get('class', 'N/A')}"))
                    sections.append(html.P(f"  • Confidence: {sq_class.get('confidence', 0):.2%}"))
            ...
            if "heart_rate_classification" in classification:
                ...
                if "class" in hr_class:
                    ...
                    sections.append(html.P(f"  • Class: {hr_class.get('class', 'N/A')}"))
                    sections.append(html.P(f"  • Confidence: {hr_class.get('confidence', 0):.2%}"))
                    sections.append(html.P(f"  • Heart Rate: {hr_class.get('heart_rate', 0):.1f} bpm"))
            ...
            if "pattern_classification" in classification:
                ...
                if "class" in pat_class:
                    ...
                    sections.append(html.P(f"  • Class: {pat_class.get('class', 'N/A')}"))
                    sections.append(html.P(f"  • Confidence: {pat_class.get('confidence', 0):.2%}"))
        ...
        if "regression" in analysis_results and "error" not in analysis_results["regression"]:
            ...
            if "linear_trend_regression" in regression and "error" not in regression["linear_trend_regression"]:
                ...
                sections.append(html.P(f"  • Trend: {lr.get('trend_direction', 'N/A')}"))
        ...
        if "clustering" in analysis_results and "error" not in analysis_results["clustering"]:
            ...
            sections.append(html.P(f"Total Clusters: {clustering.get('total_clusters', 0)}"))
            
            if "segment_clustering" in clustering and "error" not in clustering["segment_clustering"]:
                seg = clustering["segment_clustering"]
                sections.append(html.P(html.Strong("Segment Clustering:")))
                sections.append(html.P(f"  • Number of Clusters: {seg.get('n_clusters', 0)}"))
        
        # Display Dimensionality Reduction if available
        if "dimensionality_reduction" in analysis_results and "error" not in analysis_results["dimensionality_reduction"]:
            ...
            if "feature_importance" in dim_red:
                ...
                if "top_features" in fi:
                    for i, feat in enumerate(fi["top_features"][:5]):
                        sections.append(html.P(f"  • {feat.get('name', 'N/A')}: {feat.get('normalized_importance', 0):.4f}"))
        ...
        if "forecasting" in analysis_results and "error" not in analysis_results["forecasting"]:
            ...
            sections.append(html.P(f"Total Forecasts: {forecasting.get('total_forecasts', 0)}"))
            
            if "linear_forecast" in forecasting and "error" not in forecasting["linear_forecast"]:
                lf = forecasting["linear_forecast"]
                sections.append(html.P(html.Strong("Linear Forecast:")))
                sections.append(html.P(f"  • Forecast Length: {lf.get('forecast_length', 0)} samples"))
