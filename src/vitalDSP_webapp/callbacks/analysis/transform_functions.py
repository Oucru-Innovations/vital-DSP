...
def apply_fft_transform(time_data, signal_data, sampling_freq, options, window_type, n_points):
    ...
    analysis_fig.update_yaxes(title_text="Power" + (" (log)" if use_log else ""), row=2, col=1)
    ...
    if "peak_detection" in (options or []):
        ...
        peaks_idx, properties = find_peaks(magnitude, height=np.max(magnitude) * 0.1, distance=10)
...
def apply_stft_transform(
    time_data, signal_data, sampling_freq, options, window_size, overlap_percent, window_type
):
...
def apply_wavelet_transform(
    ...
    try:
        coefficients, frequencies = pywt.cwt(signal_data, scales, wavelet_type, sampling_period=1/sampling_freq)
    except Exception as e:
        logger.error(f"Wavelet transform failed: {e}")
        # Fallback to simple wavelet
        coefficients, frequencies = pywt.cwt(signal_data, scales, "morl", sampling_period=1/sampling_freq)
    ...
    analysis_fig = go.Figure(
        go.Scatter(x=frequencies, y=global_spectrum, mode="lines", name="Global Spectrum")
    )
...
def apply_hilbert_transform(time_data, signal_data, sampling_freq, options):
    ...
    results = html.Div(
        [
            ...
            html.P(f"Mean Instantaneous Frequency: {np.mean(instantaneous_frequency):.2f} Hz"),
    ...
    return html.Div(
        [
            ...
            dbc.Table(
                [
                    html.Thead(
                        html.Tr([html.Th("Band"), html.Th("Power"), html.Th("% of Total")])
                    ),
                    html.Tbody(rows),
                ],
                bordered=True,
