...
def register_respiratory_callbacks(app):
    ...
    def respiratory_analysis_callback(pathname, n_clicks, slider_value, nudge_m10, nudge_m1, nudge_p1, nudge_p10,
                                   ...
                                   preprocessing_options, low_cut, high_cut, min_breath_duration, max_breath_duration, ensemble_method):
        ...
        try:
            ...
            analysis_results = generate_comprehensive_respiratory_analysis(signal_data, time_axis, sampling_freq, 
                                                                        ...
                                                                        min_breath_duration, max_breath_duration, ensemble_method)
            ...
            sections.append(
                dbc.Card([
                    dbc.CardHeader([
                        ...
                        html.Small("Inline analysis blocks for efficient space usage", className="text-muted")
                    ]),
                    dbc.CardBody([
                        html.Div(id="resp-advanced-analysis-results")
                    ])
    ...
    @app.callback(
        ...
        [State("resp-data-store", "data"),
         State("resp-features-store", "data")]
    )
    def export_respiratory_results(n_clicks, resp_data, resp_features):
        ...
        try:
            ...
            export_data = {
                ...
                "analysis_type": "respiratory_analysis",
                "data": resp_data,
...
def generate_comprehensive_respiratory_analysis(signal_data, time_axis, sampling_freq, signal_type, 
                                              ...
                                              low_cut, high_cut, min_breath_duration, max_breath_duration, ensemble_method=None):
    ...
    try:
            ...
            for method in estimation_methods:
                ...
                try:
                    if method == "peak_detection":
                        logger.info("Computing respiratory rate using peak detection method...")
                        try:
                        ...
                        logger.info("Computing respiratory rate using zero crossing method...")
                        try:
                            rr = resp_analysis.compute_respiratory_rate(
                            ...
                            rr = rr / 2
                            logger.info(f"Zero crossing method result (corrected): {rr:.2f} BPM")
                            results.append(html.Div([
                                html.Strong("Zero Crossing Method: "),
                                html.Span(f"{rr:.2f} BPM", className="text-success"),
                                html.Small(" (corrected for symmetry)", className="text-muted ms-2")
                            ], className="mb-2"))
                        except Exception as e:
                        ...
                        if time_domain_rr is None:
                            ...
                            rr = time_domain_rr(
                                ...
                                preprocess='bandpass' if "filter" in preprocessing_options else None,
                        ...
                        if frequency_domain_rr is None:
                            ...
                            rr = rr / 2
                            logger.info(f"Frequency domain method result (corrected): {rr:.2f} BPM")
                            results.append(html.Div([
                                html.Strong("Frequency Domain Method: "),
                                html.Span(f"{rr:.2f} BPM", className="text-success"),
                                html.Small(" (corrected for symmetry)", className="text-muted ms-2")
                        ...
                        logger.info("Computing ensemble respiratory rate using all available methods...")
                        ...
                        try:
                            ...
                            if RespiratoryAnalysis is not None:
                                ...
                                try:
                                    ...
                                    ensemble_estimates.append(rr_zero / 2)  # Apply symmetry correction
                            ...
                            if frequency_domain_rr is not None:
                                try:
                                    ...
                                    ensemble_estimates.append(rr_freq / 2)  # Apply symmetry correction
                            ...
                            if len(ensemble_estimates) > 0:
                                ...
                                current_ensemble_method = ensemble_method or "mean"  # Default to mean if not specified
                                logger.info(f"Using ensemble method: {current_ensemble_method}")
                                ...
                                if current_ensemble_method == "mean":
                                    ...
                                    weights = [1.0 / (1.0 + abs(est - np.mean(ensemble_estimates))) for est in ensemble_estimates]
                                    weights = np.array(weights) / np.sum(weights)  # Normalize weights
                                    ensemble_result = np.average(ensemble_estimates, weights=weights)
                                    method_description = "Weighted Average (by reliability)"
                                elif current_ensemble_method == "bagging":
                                    # Bootstrap aggregation
                                    n_bootstrap = 100
                                    bootstrap_estimates = []
                                    for _ in range(n_bootstrap):
                                        indices = np.random.choice(len(ensemble_estimates), size=len(ensemble_estimates), replace=True)
                                        bootstrap_sample = [ensemble_estimates[i] for i in indices]
                                    ...
                                    ensemble_result = ensemble_estimates[0]  # Start with first estimate
                                    for i in range(1, len(ensemble_estimates)):
                                        # Simple boosting: adjust based on previous estimate
                                        ensemble_result = 0.7 * ensemble_result + 0.3 * ensemble_estimates[i]
                                ...
                                ensemble_median = np.median(ensemble_estimates)
                                ensemble_min = np.min(ensemble_estimates)
                                ensemble_max = np.max(ensemble_estimates)
                                
                                # Calculate confidence interval (95%)
                                ensemble_ci_lower = np.percentile(ensemble_estimates, 2.5)
                                ensemble_ci_upper = np.percentile(ensemble_estimates, 97.5)
                                
                                # Calculate coefficient of variation
                                ensemble_cv = ensemble_std / ensemble_result if ensemble_result > 0 else 0
                                ...
                                results.append(html.Div([
                                    html.Strong("🎯 Ensemble Respiratory Rate: "),
                                    html.Span(f"{ensemble_result:.2f} ± {ensemble_std:.2f} BPM", className="text-success"),
                                    html.Br(),
                                    html.Small(f"Ensemble Method: {method_description}", className="text-info"),
                                    html.Br(),
                                    html.Small(f"Reliability: ", className="text-muted"),
                                    html.Span(f"{reliability}", className=reliability_color),
                                    html.Br(),
                                    html.Small(f"Methods used: {len(ensemble_estimates)} ({', '.join(method_names)})", className="text-muted"),
                                    html.Br(),
                                    html.Small(f"Range: {ensemble_min:.1f} - {ensemble_max:.1f} BPM", className="text-muted"),
                                    html.Br(),
                                    html.Small(f"95% CI: {ensemble_ci_lower:.1f} - {ensemble_ci_upper:.1f} BPM", className="text-muted"),
                                    html.Br(),
                                    html.Small(f"Coefficient of Variation: {ensemble_cv:.3f}", className="text-muted")
                                ], className="mb-3 p-3 border border-success rounded"))
                                
                                logger.info(f"Ensemble method result: {ensemble_result:.2f} ± {ensemble_std:.2f} BPM using {current_ensemble_method}")
                            else:
                                results.append(html.Div([
                                    html.Strong("Ensemble Method: "),
                                    html.Span("No methods available", className="text-warning")
                                ], className="mb-2"))
        ...
        if estimation_methods and len(estimation_methods) > 1:
            ...
            results.append(html.H5("📊 METHOD AGREEMENT ANALYSIS", className="text-primary mb-3"))
            ...
            try:
                ...
                try:
                    ...
                    if len(peaks_auto) > 0:
                        ...
                        method_results.append(("Autocorrelation", rr_auto, len(peaks_auto)))
                except Exception as e:
                ...
                if len(method_results) > 1:
                    ...
                    results.append(html.Div([
                        ...
                        html.Div([
                            html.H6("📊 METHOD RESULTS", className="text-dark mb-2"),
                            html.P(f"Methods Used: {len(method_results)}", className="mb-1"),
                            html.P(f"Mean RR: {mean_rr:.1f} BPM", className="mb-1"),
                            html.P(f"Std Dev: {std_rr:.1f} BPM", className="mb-1"),
                            html.P(f"Coefficient of Variation: {cv_rr:.3f}", className="mb-1")
                        ], className="p-2 border border-primary rounded mb-2"),
                        
                        # Individual Method Results
                        html.Div([
                            html.H6("🔬 INDIVIDUAL METHODS", className="text-dark mb-2"),
                            *[html.P(f"{method[0]}: {method[1]:.1f} BPM ({method[2]} samples)", className="mb-1") 
                              for method in method_results]
                        ], className="p-2 border border-info rounded mb-2"),
                        
                        # Agreement Assessment
                        html.Div([
                            html.H6("🎯 AGREEMENT ASSESSMENT", className="text-dark mb-2"),
                            html.P(f"Agreement Level: {agreement_level}", className=f"mb-1 {agreement_color}"),
                            html.P(f"Mean Difference: {mean_diff:.1f} BPM", className="mb-1"),
                            html.P(f"Max Difference: {max_diff:.1f} BPM", className="mb-1"),
                            html.P(f"CV: {cv_rr:.3f}", className="mb-1")
                        ], className="p-2 border border-success rounded mb-2"),
                        
                        # Clinical Interpretation
                        html.Div([
                            html.H6("🏥 CLINICAL INTERPRETATION", className="text-dark mb-2"),
                            html.P(f"Reliability: {'High' if cv_rr < 0.2 else 'Moderate' if cv_rr < 0.3 else 'Low'}", 
                                   className="text-success" if cv_rr < 0.2 else "text-warning" if cv_rr < 0.3 else "text-danger"),
                            html.P(f"Consistency: {'Consistent' if mean_diff < 2 else 'Variable' if mean_diff < 5 else 'Inconsistent'}", 
                                   className="text-success" if mean_diff < 2 else "text-warning" if mean_diff < 5 else "text-danger"),
                            html.P(f"Recommendation: {'Use any method' if cv_rr < 0.15 else 'Use average' if cv_rr < 0.25 else 'Verify manually'}", 
                                   className="text-success" if cv_rr < 0.25 else "text-info" if cv_rr < 0.25 else "text-warning")
                        ], className="p-2 border border-warning rounded")
                        
                    ], className="mb-3"))
                    
                    logger.info(f"Method agreement analysis completed: {len(method_results)} methods, CV: {cv_rr:.3f}")
                    
                else:
                    results.append(html.Div([
                        html.H6("⚠️ INSUFFICIENT METHODS", className="text-warning mb-2"),
                        html.P(f"Only {len(method_results)} method(s) available. Need at least 2 methods for agreement analysis."),
                        html.P("Try different signal processing approaches or check signal quality.")
        ...
        if advanced_options:
            ...
            if "sleep_apnea" in advanced_options:
                ...
                try:
                    ...
                    if total_apnea_events > 0:
                        ...
                        apnea_card = html.Div([
                            html.H6("😴 SLEEP APNEA", className="text-warning mb-1"),
                            html.P(f"Events: {total_apnea_events}", className="mb-1 fw-bold"),
                            html.P(f"Amplitude: {len(apnea_events)}", className="mb-1 small"),
                            html.P(f"Pause: {len(pause_apnea_events)}", className="mb-1 small"),
                            html.P(f"First: {all_events[0][0]:.1f}s", className="mb-0 small text-muted")
                        ...
                        no_apnea_card = html.Div([
                            html.H6("😴 SLEEP APNEA", className="text-success mb-1"),
                            html.P("No events detected", className="mb-0 fw-bold text-success")
                    ...
                    error_card = html.Div([
                        ...
                        html.P("Analysis failed", className="mb-0 fw-bold text-danger")
            ...
            if "breathing_pattern" in advanced_options:
                ...
                try:
                    ...
                    if len(peaks) > 1:
                        ...
                        median_interval = np.median(breath_intervals)
                        ...
                        min_interval = np.min(breath_intervals)
                        max_interval = np.max(breath_intervals)
                        interval_range = max_interval - min_interval
                        
                        # Detect breathing irregularities
                        irregular_breaths = np.sum(np.abs(breath_intervals - mean_interval) > 2 * variability)
                        irregularity_percentage = (irregular_breaths / len(breath_intervals)) * 100
                        ...
                        breathing_card = html.Div([
                            html.H6("🫁 BREATHING PATTERN", className="text-primary mb-1"),
                            html.P(f"Breaths: {len(peaks)}", className="mb-1 fw-bold"),
                            html.P(f"RR: {rr_bpm:.1f} BPM", className="mb-1"),
                            html.P(f"Pattern: {pattern_type}", className=f"mb-1 {pattern_color}"),
                            html.P(f"Stability: {stability}", className=f"mb-1 {stability_color}"),
                            html.P(f"CV: {rr_cv:.3f}", className="mb-0 small text-muted")
                        ], className="p-2 border border-primary rounded bg-light h-100")
                        analysis_blocks.append(breathing_card)
                        
                        logger.info(f"Breathing pattern analysis completed: {len(peaks)} breaths, RR: {rr_bpm:.1f} BPM, CV: {rr_cv:.3f}")
                        
                    else:
                        # Create insufficient data card
                        insufficient_card = html.Div([
                            html.H6("🫁 BREATHING PATTERN", className="text-warning mb-1"),
                            html.P(f"Only {len(peaks)} breath(s)", className="mb-1 fw-bold text-warning"),
                            html.P("Need at least 2 breaths", className="mb-0 small text-muted")
                        ], className="p-2 border border-warning rounded bg-light h-100")
                    ...
                    # Create error card
                    error_card = html.Div([
                        html.H6("🫁 BREATHING PATTERN", className="text-danger mb-1"),
                        html.P("Analysis failed", className="mb-1 fw-bold text-danger"),
                        html.P(f"Error: {str(e)[:30]}...", className="mb-0 small text-muted")
                    ], className="p-2 border border-danger rounded bg-light h-100")
                    analysis_blocks.append(error_card)
                        ...
                        if PPGRespiratoryAutonomicFeatures is None:
                            ...
                            try:
                                ...
                                if len(peaks) > 1:
                                    ...
                                    rrv_norm = rrv / np.mean(breath_intervals)  # Normalized RRV
                                    
                                    results.append(html.Div([
                                        html.H5("📊 RESPIRATORY RATE VARIABILITY", className="text-primary mb-3"),
                                        html.Div([
                                            html.H6("🫁 BASIC RRV METRICS", className="text-dark mb-2"),
                                            html.P(f"RRV (Std Dev): {rrv:.3f}s", className="mb-1"),
                                            html.P(f"Normalized RRV: {rrv_norm:.3f}", className="mb-1"),
                                            html.P(f"Mean RR: {rr_bpm:.1f} BPM", className="mb-1"),
                                            html.P(f"Breaths: {len(peaks)}", className="mb-1"),
                                            html.P("Method: Fallback (Peak Detection)", className="text-info")
                                        ], className="p-3 border border-info rounded bg-light")
                                    ], className="mb-4"))
                                else:
                                    results.append(html.Div([
                                        html.H5("📊 RESPIRATORY RATE VARIABILITY", className="text-warning mb-3"),
                                        html.Div([
                                            html.H6("⚠️ INSUFFICIENT DATA", className="text-warning"),
                                            html.P("Need at least 2 breaths for RRV analysis.", className="mb-1")
                                        ], className="p-3 border border-warning rounded bg-light")
                                    ], className="mb-4"))
                            except Exception as fallback_error:
                                logger.error(f"Fallback RRV calculation failed: {fallback_error}")
                                results.append(html.Div([
                                    html.H5("📊 RESPIRATORY RATE VARIABILITY", className="text-danger mb-3"),
                                    html.Div([
                                        html.H6("❌ ANALYSIS FAILED", className="text-danger"),
                                        html.P(f"Error: {str(fallback_error)}", className="mb-1")
                                    ], className="p-3 border border-danger rounded bg-light")
                                ], className="mb-4"))
                        else:
                            ...
                            try:
                                ...
                                ppg_rrv_card = html.Div([
                                    html.H6("📊 PPG RRV", className="text-success mb-1"),
                                    html.P(f"RRV: {rrv:.4f}s", className="mb-1 fw-bold"),
                                    html.P(f"RSA: {rsa:.4f}s", className="mb-1"),
                                    html.P("vitalDSP PPG", className="mb-0 small text-success")
                                ], className="p-2 border border-success rounded bg-light h-100")
                                ...
                                results.append(html.Div([
                                    html.H5("📊 RESPIRATORY RATE VARIABILITY", className="text-danger mb-3"),
                                    html.Div([
                                        html.H6("❌ PPG ANALYSIS FAILED", className="text-danger"),
                                        html.P(f"Error: {str(ppg_error)}", className="mb-1"),
                                        html.P("Falling back to basic RRV calculation...", className="text-info")
                                    ], className="p-3 border border-danger rounded bg-light")
                                ], className="mb-4"))
                    else:
                        ...
                        try:
                            ...
                            if len(peaks) > 1:
                                ...
                                basic_rrv_card = html.Div([
                                    html.H6("📊 BASIC RRV", className="text-info mb-1"),
                                    html.P(f"RRV: {rrv:.3f}s", className="mb-1 fw-bold"),
                                    html.P(f"Norm RRV: {rrv_norm:.3f}", className="mb-1"),
                                    html.P(f"RR: {rr_bpm:.1f} BPM", className="mb-1"),
                                    html.P(f"Breaths: {len(peaks)}", className="mb-0 small text-muted")
                                ...
                                insufficient_rrv_card = html.Div([
                                    html.H6("📊 BASIC RRV", className="text-warning mb-1"),
                                    html.P("Insufficient data", className="mb-1 fw-bold text-warning"),
                                    html.P("Need at least 2 breaths", className="mb-0 small text-muted")
                                ], className="p-2 border border-warning rounded bg-light h-100")
                            ...
                            error_rrv_card = html.Div([
                                html.H6("📊 BASIC RRV", className="text-danger mb-1"),
                                html.P("Analysis failed", className="mb-1 fw-bold text-danger"),
                                html.P(f"Error: {str(basic_error)[:30]}...", className="mb-0 small text-muted")
                            ], className="p-2 border border-danger rounded bg-light h-100")
                    ...
                    if ppg_ecg_fusion is None:
                        ...
                        try:
                            ...
                            if len(peaks) > 1:
                                ...
                                results.append(html.Div([
                                    html.H5("🔗 PPG-ECG FUSION", className="text-primary mb-3"),
                                    html.Div([
                                        html.H6("🫁 FUSION RESULTS", className="text-dark mb-2"),
                                        html.P(f"Fusion RR: {fusion_rr:.1f} ± {fusion_std:.1f} BPM", className="mb-1"),
                                        html.P(f"PPG RR: {rr_ppg:.1f} BPM", className="mb-1"),
                                        html.P(f"Frequency RR: {rr_freq:.1f} BPM", className="mb-1"),
                                        html.P("Method: Fallback (Peak + FFT)", className="text-info")
                                    ], className="p-3 border border-info rounded bg-light")
                                ], className="mb-4"))
                            else:
                                results.append(html.Div([
                                    ...
                                    html.Div([
                                        html.Strong("⚠️ Insufficient Data:", className="text-warning"),
                                        html.Br(),
                                        html.Small("Need at least 2 breaths for fusion analysis.", className="text-muted")
                                    ], className="mb-3 p-3 border border-warning rounded bg-light")
                                ], className="mb-4"))
                        except Exception as fallback_error:
                            logger.error(f"Fallback PPG-ECG fusion failed: {fallback_error}")
                            results.append(html.Div([
                                html.H6("🔗 PPG-ECG Fusion Analysis", className="mb-3"),
                                html.Div([
                                    html.Strong("❌ Analysis Failed:", className="text-danger"),
                                    html.Br(),
                                    html.Small(f"Error: {str(fallback_error)}", className="text-muted")
                        ...
                        try:
                            ...
                            results.append(html.Div([
                                html.H5("🔗 PPG-ECG FUSION", className="text-success mb-3"),
                                html.Div([
                                    html.H6("🫁 VITALDSP RESULT", className="text-dark mb-2"),
                                    html.P(f"Fusion RR: {fusion_rr:.1f} BPM", className="mb-1"),
                                    html.P("Method: vitalDSP PPG-ECG Fusion", className="text-success")
                                ], className="p-3 border border-success rounded bg-light")
                            ], className="mb-4"))
                        except Exception as fusion_error:
                            logger.error(f"vitalDSP PPG-ECG fusion failed: {fusion_error}")
                            results.append(html.Div([
                                html.H6("🔗 PPG-ECG Fusion Analysis", className="mb-3"),
                                html.Div([
                                    html.Strong("❌ vitalDSP Fusion Failed:", className="text-danger"),
                                    html.Br(),
                                    html.Small(f"Error: {str(fusion_error)}", className="text-muted"),
                                    html.Br(),
                                    html.Small("Falling back to basic fusion...", className="text-info")
                                ], className="mb-3 p-3 border border-danger rounded bg-light")
                            ], className="mb-4"))
                    ...
                    if respiratory_cardiac_fusion is None:
                        ...
                        try:
                            ...
                            if len(peaks) > 1:
                                ...
                                results.append(html.Div([
                                    html.H6("🫀 Respiratory-Cardiac Fusion Analysis", className="mb-3"),
                                    html.Div([
                                        html.Strong("🫁 Fusion Respiratory Rate:", className="text-primary"),
                                        html.Br(),
                                        html.Small(f"• Fusion RR: {fusion_rr:.1f} ± {fusion_std:.1f} BPM", className="text-muted"),
                                        html.Br(),
                                        html.Small(f"• Respiratory-based RR: {rr_resp:.1f} BPM", className="text-muted"),
                                        html.Br(),
                                        html.Small(f"• Cardiac-based RR: {rr_cardiac:.1f} BPM", className="text-muted"),
                                        html.Br(),
                                        html.Small(f"• Analysis Method: Fallback (Peak + FFT)", className="text-info")
                            ...
                            logger.error(f"Fallback respiratory-cardiac fusion failed: {fallback_error}")
                            results.append(html.Div([
                                html.H6("🫀 Respiratory-Cardiac Fusion Analysis", className="mb-3"),
                                html.Div([
                                    html.Strong("❌ Analysis Failed:", className="text-danger"),
                        ...
                        try:
                            ...
                            results.append(html.Div([
                                html.H6("🫀 Respiratory-Cardiac Fusion Analysis", className="mb-3"),
                                html.Div([
                                    html.Strong("🫁 vitalDSP Fusion Result:", className="text-primary"),
                                    html.Br(),
                                    html.Small(f"• Fusion Respiratory Rate: {fusion_rr:.1f} BPM", className="text-muted"),
                                    html.Br(),
                                    html.Small(f"• Analysis Method: vitalDSP Respiratory-Cardiac Fusion", className="text-success")
                                ], className="mb-3 p-3 border border-success rounded bg-light")
                            ], className="mb-4"))
                        except Exception as fusion_error:
                            logger.error(f"vitalDSP respiratory-cardiac fusion failed: {fusion_error}")
                            results.append(html.Div([
                                html.H6("🫀 Respiratory-Cardiac Fusion Analysis", className="mb-3"),
                                html.Div([
                                    html.Strong("❌ vitalDSP Fusion Failed:", className="text-danger"),
            ...
            if "multimodal" in advanced_options:
                ...
                try:
                    if multimodal_analysis is None:
                        ...
                        try:
                            ...
                            if len(peaks) > 1:
                                ...
                                results.append(html.Div([
                                    html.H5("🔗 MULTIMODAL FUSION", className="text-primary mb-3"),
                                    html.Div([
                                        html.H6("🫁 MULTIMODAL RESULTS", className="text-dark mb-2"),
                                        html.P(f"Multimodal RR: {multimodal_rr:.1f} ± {multimodal_std:.1f} BPM", className="mb-1"),
                                        html.P(f"Peak-based RR: {rr_peak:.1f} BPM", className="mb-1"),
                                        html.P(f"Frequency-based RR: {rr_freq:.1f} BPM", className="mb-1"),
                                        html.P("Method: Fallback (Peak + FFT)", className="text-info")
                                    ], className="p-3 border border-info rounded bg-light")
                                ], className="mb-4"))
                            else:
                                results.append(html.Div([
                                    html.H5("🔗 MULTIMODAL FUSION", className="text-warning mb-3"),
                                    html.Div([
                                        html.H6("⚠️ INSUFFICIENT DATA", className="text-warning"),
                                        html.P("Need at least 2 breaths for multimodal analysis.", className="mb-1")
                                    ], className="p-3 border border-warning rounded bg-light")
                                ], className="mb-4"))
                        except Exception as fallback_error:
                            logger.error(f"Fallback multimodal fusion failed: {fallback_error}")
                            results.append(html.Div([
                                html.H5("🔗 MULTIMODAL FUSION", className="text-danger mb-3"),
                                html.Div([
                                    html.H6("❌ ANALYSIS FAILED", className="text-danger"),
                                    html.P(f"Error: {str(fallback_error)}", className="mb-1")
                                ], className="p-3 border border-danger rounded bg-light")
                        ...
                        try:
                            ...
                            results.append(html.Div([
                                html.H5("🔗 MULTIMODAL FUSION", className="text-success mb-3"),
                                html.Div([
                                    html.H6("🫁 VITALDSP RESULT", className="text-dark mb-2"),
                                    html.P(f"Multimodal RR: {multimodal_rr:.1f} BPM", className="mb-1"),
                                    html.P("Method: vitalDSP Multimodal Analysis", className="text-success")
                                ], className="p-3 border border-success rounded bg-light")
                            ], className="mb-4"))
                        except Exception as fusion_error:
                            logger.error(f"vitalDSP multimodal fusion failed: {fusion_error}")
                            results.append(html.Div([
                                html.H5("🔗 MULTIMODAL FUSION", className="text-danger mb-3"),
                                html.Div([
                                    html.H6("❌ VITALDSP FAILED", className="text-danger"),
                                    html.P(f"Error: {str(fusion_error)}", className="mb-1"),
                                    html.P("Falling back to basic multimodal...", className="text-info")
                                ], className="p-3 border border-danger rounded bg-light")
                            ], className="mb-4"))
                    ...
                    results.append(html.Div([
                        html.H6("🔗 Multimodal Fusion Analysis", className="mb-3"),
                        html.Div([
                            html.Strong("❌ Analysis Failed:", className="text-danger"),
                            html.Br(),
                            html.Small(f"Error: {str(e)}", className="text-muted")
            ...
            if "quality_assessment" in advanced_options:
                ...
                try:
                    ...
                    rms_noise_signal = np.sqrt(np.mean((signal_data - signal_mean)**2))
                    snr_rms = 20 * np.log10(rms_signal / rms_noise_signal) if rms_noise_signal > 0 else 0
                    ...
                    signal_entropy = -np.sum(signal_data * np.log(np.abs(signal_data) + 1e-10))
                    zero_crossings = np.sum(np.diff(np.sign(signal_data)) != 0)
                    
                    results.append(html.Div([
                        html.H5("🎯 SIGNAL QUALITY ASSESSMENT", className="text-primary mb-2"),
                        
                        # Quality Summary
                        html.Div([
                            html.H6("📊 QUALITY SUMMARY", className="text-dark mb-2"),
                            html.P(f"Overall Quality: {quality_level}", className=f"mb-1 {quality_color}"),
                            html.P(f"Variance SNR: {snr_variance:.2f} dB", className="mb-1"),
                            html.P(f"Peak-to-Peak SNR: {snr_pp:.2f} dB", className="mb-1"),
                            html.P(f"RMS SNR: {snr_rms:.2f} dB", className="mb-1")
                        ], className="p-2 border border-primary rounded mb-2"),
                        
                        # Signal Statistics
                        html.Div([
                            html.H6("📈 SIGNAL STATISTICS", className="text-dark mb-2"),
                            html.P(f"Dynamic Range: {dynamic_range:.3f}", className="mb-1"),
                            html.P(f"Signal Power: {signal_power:.3f}", className="mb-1"),
                            html.P(f"Signal Energy: {signal_energy:.3f}", className="mb-1"),
                            html.P(f"Zero Crossings: {zero_crossings}", className="mb-1")
                        ], className="p-2 border border-info rounded mb-2"),
                        
                        # Signal Characteristics
                        html.Div([
                            html.H6("🔍 SIGNAL CHARACTERISTICS", className="text-dark mb-2"),
                            html.P(f"Mean: {signal_mean:.3f}", className="mb-1"),
                            html.P(f"Std Dev: {signal_std:.3f}", className="mb-1"),
                            html.P(f"Range: {signal_min:.3f} to {signal_max:.3f}", className="mb-1"),
                            html.P(f"CV: {signal_std/abs(signal_mean):.3f}" if abs(signal_mean) > 0 else "N/A", className="mb-1")
                        ], className="p-2 border border-success rounded")
                        
                    ], className="mb-3"))
                    
                    logger.info(f"Signal quality assessment completed: SNR={snr_variance:.2f} dB, Quality={quality_level}")
                    
                except Exception as e:
                    logger.error(f"Quality assessment failed: {e}")
                    results.append(html.Div([
                        html.H5("🎯 SIGNAL QUALITY ASSESSMENT", className="text-danger mb-3"),
                        html.Div([
                            html.H6("❌ ANALYSIS FAILED", className="text-danger"),
        ...
        if analysis_blocks:
            results.append(html.Div([
                ...
                html.Div([
                    dbc.Row([
                        dbc.Col(block, md=3, className="mb-2") for block in analysis_blocks
                    ], className="g-2")
                ])
            ], className="mb-3"))
        ...
        results.append(html.Hr())
        results.append(html.H5("🔍 ENHANCED SIGNAL ANALYSIS", className="text-primary mb-2"))
        ...
        results.append(html.Div([
            ...
            html.Div([
                ...
                html.Br(),
                html.Small(f"Crest Factor: {crest_factor:.2f}", className="text-muted"),
                html.Br(),
                html.Small(f"RMS: {rms_val:.3f}", className="text-muted"),
                html.Br(),
                html.Small(f"Peak-to-Peak: {peak_to_peak:.3f}", className="text-muted")
            ], className="ms-3 mb-2"),
            
            # Frequency Analysis
            html.H6("🌊 Frequency Analysis", className="mb-2"),
            html.Div([
                html.Small(f"Dominant Frequency: {dominant_freq:.3f} Hz ({dominant_freq_bpm:.1f} BPM)", className="text-muted"),
                html.Br(),
                html.Small(f"Respiratory Power Ratio: {resp_power_ratio:.1%}", className="text-muted")
            ], className="ms-3 mb-2"),
            
            # Breathing Pattern Analysis
            html.H6("🫁 Breathing Pattern Analysis", className="mb-2"),
            html.Div([
                html.Small(f"Pattern Type: ", className="text-muted"),
                html.Span(f"{pattern_type}", className=pattern_color),
                html.Br(),
                html.Small(f"Mean Interval: {mean_interval:.2f}s", className="text-muted"),
                html.Br(),
                html.Small(f"Interval CV: {cv_intervals:.3f}", className="text-muted")
            ], className="ms-3 mb-2"),
            
            # Clinical Interpretation
            html.H6("🏥 Clinical Interpretation", className="mb-2"),
            html.Div([
                html.Small(f"Status: ", className="text-muted"),
        ...
        try:
            ...
            if rr_estimates:
                ...
                results.append(html.Div([
                    ...
                    html.Div([
                        html.Small(f"Mean Respiratory Rate: ", className="text-muted"),
                        html.Span(f"{overall_mean:.1f} ± {overall_std:.1f} BPM", className="text-success"),
                        html.Br(),
                        html.Small(f"Method Agreement: ", className="text-muted"),
                        html.Span(f"{assessment}", className=assessment_color),
                        html.Br(),
                        html.Small(f"Number of Methods: {len(rr_estimates)}", className="text-muted")
                    ], className="ms-3 mb-2")
        ...
        results.append(html.Div([
            ...
            html.Div([
                html.Small("• Use ensemble method for most reliable estimates", className="text-muted"),
                html.Br(),
                html.Small("• Check signal quality if methods disagree significantly", className="text-muted"),
                html.Br(),
                html.Small("• Consider preprocessing options for noisy signals", className="text-muted"),
                html.Br(),
                html.Small("• Monitor breathing pattern regularity for clinical insights", className="text-muted")
            ], className="ms-3 mb-2")
        ], className="p-3 border border-info rounded mb-3"))
    ...
    try:
        ...
        fig = make_subplots(
            ...
            subplot_titles=('Time Domain Signal', 'Frequency Domain', 
                           'Breathing Pattern & RRV', 'Respiratory Rate Over Time',
                           'Sleep Apnea Detection', 'Signal Quality & Ensemble'),
                ...
                if len(peaks) > 0:
                    ...
                    if len(peaks) > 1:
                        ...
                        if len(rr_over_time) > 2:
                            ...
                            if window_size > 1:
                                rr_moving_avg = np.convolve(rr_over_time, np.ones(window_size)/window_size, mode='same')
                                fig.add_trace(
                                    go.Scatter(x=interval_times, y=rr_moving_avg,
                                             mode='lines', name='RR Moving Average',
        ...
        try:
            ...
            if window_size > 0:
                ...
                if estimation_methods and len(estimation_methods) > 1 and "ensemble" in estimation_methods:
                    try:
                        ...
                        if ensemble_window > 0 and len(signal_data) > ensemble_window:
                            ...
                            for i in range(n_windows):
                                ...
                                window_times.append(time_axis[start_idx + ensemble_window // 2])
                            
                            # Add ensemble stability indicator
                            fig.add_trace(
                                go.Scatter(x=window_times, y=window_means,
                                         mode='lines+markers', name='Local Ensemble Mean',
                            ...
                            fig.add_trace(
                                go.Scatter(x=window_times, y=window_stds,
                                         mode='lines', name='Local Ensemble Std',
