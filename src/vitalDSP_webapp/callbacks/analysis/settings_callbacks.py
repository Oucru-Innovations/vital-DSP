...
from vitalDSP_webapp.services.settings_service import (
    get_settings_service, get_current_settings, update_settings
)
from vitalDSP_webapp.utils.settings_utils import (
    ThemeManager, SystemMonitor, 
    SettingsValidator, SettingsExporter, get_system_recommendations
)
...
def register_settings_callbacks(app):
    ...
    def settings_management_callback(save_clicks, reset_clicks, export_clicks, 
                                   import_clicks, pathname, theme, timezone,
                                   page_size, auto_refresh, display_options,
                                   sampling_freq, fft_points, window_type, peak_threshold,
                                   quality_threshold, analysis_options, max_file_size,
                                   auto_save, data_retention, export_format, image_format,
                                   export_options, cpu_usage, memory_limit, parallel_threads,
                                   session_timeout, security_options):
        ...
        try:
            ...
            if button_id == "settings-save-btn" and save_clicks:
                return handle_save_settings(
                    theme, timezone, page_size, auto_refresh, display_options,
                    sampling_freq, fft_points, window_type, peak_threshold,
                    quality_threshold, analysis_options, max_file_size,
                    auto_save, data_retention, export_format, image_format,
                    export_options, cpu_usage, memory_limit, parallel_threads,
...
def handle_save_settings(theme, timezone, page_size, auto_refresh, display_options,
                        sampling_freq, fft_points, window_type, peak_threshold,
                        quality_threshold, analysis_options, max_file_size,
                        auto_save, data_retention, export_format, image_format,
                        export_options, cpu_usage, memory_limit, parallel_threads,
    ...
    try:
        ...
        status_display = create_success_display("Settings Saved Successfully", 
                                             "All settings have been updated and saved.")
        
        return status_display, settings_summary, current_settings
        
    except Exception as e:
        logger.error(f"Error saving settings: {e}")
        error_display = create_error_display("Save Error", f"Failed to save settings: {str(e)}")
...
def handle_reset_settings():
    ...
    try:
        ...
        if success:
            ...
            status_display = create_success_display("Settings Reset", 
                                                 "All settings have been reset to default values.")
            
            return status_display, settings_summary, current_settings
        else:
            error_display = create_error_display("Reset Error", "Failed to reset settings.")
            return error_display, {}, {}
            
    except Exception as e:
        logger.error(f"Error resetting settings: {e}")
        error_display = create_error_display("Reset Error", f"Failed to reset settings: {str(e)}")
        return error_display, {}, {}
...
def handle_export_settings():
    ...
    try:
        ...
        try:
            ...
            status_display = create_success_display("Settings Exported", 
                                                 f"Settings exported to {filename}")
        except Exception as export_error:
            logger.error(f"Export error: {export_error}")
            status_display = create_error_display("Export Error", f"Failed to export: {str(export_error)}")
        
        return status_display, settings_dict, {}
        
    except Exception as e:
        logger.error(f"Error handling export: {e}")
        error_display = create_error_display("Export Error", f"Failed to handle export: {str(e)}")
        return error_display, {}, {}
...
def handle_import_settings():
    ...
    try:
        ...
        status_display = create_info_display("Import Feature", 
                                           "Settings import functionality requires file upload integration.")
        return status_display, {}, {}
        
    except Exception as e:
        logger.error(f"Error handling import: {e}")
        error_display = create_error_display("Import Error", f"Failed to handle import: {str(e)}")
        return error_display, {}, {}
    ...
    try:
        ...
        return html.Div([
            ...
            html.Div([
                ...
                html.P(f"Sampling Freq: {summary['analysis']['sampling_freq']} Hz"),
                html.P(f"FFT Points: {summary['analysis']['fft_points']}"),
                html.P(f"Window Type: {summary['analysis']['window_type'].title()}")
            ], className="mb-3"),
            
            # System Health
            html.Div([
                html.H6("🖥️ System Health", className=f"text-{system_health.get('color', 'info')}"),
                html.P(f"Overall Score: {system_health.get('overall_score', 'N/A')}/100"),
                html.P(f"Status: {system_health.get('status', 'N/A').title()}"),
                html.P(f"Memory: {system_health.get('memory_score', 'N/A')}/100"),
                html.P(f"CPU: {system_health.get('cpu_score', 'N/A')}/100")
            ], className="mb-3"),
            
            html.Small(f"Last Updated: {summary['last_updated']}", className="text-muted")
        ])
    ...
    try:
        ...
        return html.Div([
            ...
            html.Div([
                ...
                html.P(f"Architecture: {system_info.get('architecture', 'N/A')}"),
            ...
            html.Div([
                ...
                html.P(f"Used: {memory_info.get('used_gb', 0)} GB ({memory_info.get('percent_used', 0)}%)"),
            ...
            html.Div([
                ...
                html.P(f"Used: {disk_info.get('used_gb', 0)} GB ({disk_info.get('percent_used', 0)}%)"),
                html.P(f"Free: {disk_info.get('free_gb', 0)} GB")
            ], className="mb-3"),
            
            # System Health
            html.Div([
                html.H6(f"System Health: {system_health.get('status', 'N/A').title()}", 
                       className=f"text-{system_health.get('color', 'info')}"),
                html.P(f"Overall Score: {system_health.get('overall_score', 'N/A')}/100"),
                html.Small(f"Updated: {system_health.get('timestamp', 'N/A')}", className="text-muted")
            ])
        ])
...
def create_settings_validation_display():
    ...
    try:
        ...
        return html.Div([
            html.H6("✅ Settings Validation Results", className="text-primary mb-3"),
            ...
            html.Div([
                ...
                html.Div([
                    html.P(f"Status: {'✅ Valid' if general_validation['valid'] else '❌ Invalid'}"),
                    *[html.P(f"❌ Error: {error}") for error in general_validation.get('errors', [])],
                    *[html.P(f"⚠️ Warning: {warning}") for warning in general_validation.get('warnings', [])],
                    *[html.P(f"💡 Recommendation: {rec}") for rec in general_validation.get('recommendations', [])]
                ])
            ], className="mb-3"),
            
            # Analysis Settings Validation
            html.Div([
                html.H6("📊 Analysis Settings", 
                       className=f"text-{'success' if analysis_validation['valid'] else 'danger'}"),
                html.Div([
                    html.P(f"Status: {'✅ Valid' if analysis_validation['valid'] else '❌ Invalid'}"),
                    *[html.P(f"❌ Error: {error}") for error in analysis_validation.get('errors', [])],
                    *[html.P(f"⚠️ Warning: {warning}") for warning in analysis_validation.get('warnings', [])],
                    *[html.P(f"💡 Recommendation: {rec}") for rec in analysis_validation.get('recommendations', [])]
                ])
            ], className="mb-3"),
            ...
            html.Div([
                ...
                html.Div([
                    html.P(f"Status: {'✅ Valid' if data_validation['valid'] else '❌ Invalid'}"),
                    *[html.P(f"❌ Error: {error}") for error in data_validation.get('errors', [])],
                    *[html.P(f"⚠️ Warning: {warning}") for warning in data_validation.get('warnings', [])],
                    *[html.P(f"💡 Recommendation: {rec}") for rec in data_validation.get('recommendations', [])]
                ])
            ], className="mb-3"),
            ...
            html.Div([
                ...
                html.Div([
                    html.P(f"Status: {'✅ Valid' if system_validation['valid'] else '❌ Invalid'}"),
                    *[html.P(f"❌ Error: {error}") for error in system_validation.get('errors', [])],
                    *[html.P(f"⚠️ Warning: {warning}") for warning in system_validation.get('warnings', [])],
                    *[html.P(f"💡 Recommendation: {rec}") for rec in system_validation.get('recommendations', [])]
                ])
            ])
        ])
...
def create_theme_preview_display(theme):
    ...
    try:
        ...
        return html.Div([
            html.H6(f"🎨 {theme.title()} Theme Preview", className="text-primary mb-3"),
            ...
            html.Div([
                ...
                html.Div([
                    html.Div([
                        ...
                        html.Span(f"{key.replace('_', ' ').title()}: {color}")
                    ], className="mb-2") for key, color in colors.items()
                ])
            ], className="mb-3"),
            
            html.P(f"Description: {theme_info['description']}", className="text-muted")
        ])
...
def create_settings_recommendations_display():
    ...
    try:
        ...
        if current_settings.general.page_size > 50:
            recommendations.append("Large page sizes may impact performance on slower devices")
        ...
        if current_settings.system.parallel_threads > 8:
            recommendations.append("High parallel processing may impact system responsiveness")
        ...
        return html.Div([
            ...
            html.Div([
                ...
                html.Ul([
                    html.Li(rec) for rec in recommendations[:5]  # Show first 5
                ])
            ], className="mb-3"),
            ...
            html.Div([
                ...
                html.Ul([
                    html.Li("Use appropriate FFT sizes for your signal characteristics"),
                    html.Li("Balance parallel processing with system resources"),
                    html.Li("Consider data retention policies for storage optimization"),
                    html.Li("Monitor system health regularly")
                ])
            ], className="mb-3"),
            
            html.Small("Recommendations are based on current system capabilities and best practices", 
...
def create_error_display(title, message):
    ...
    return html.Div([
        ...
        html.Small("Please check the settings and try again", className="text-muted")
    ])
