...
def register_health_report_callbacks(app):
    ...
    @app.callback(
        ...
        [Input("health-report-generate-btn", "n_clicks"),
         Input("url", "pathname")],
    ...
    def health_report_generation_callback(n_clicks, pathname, report_type, sections, 
                                         customization, format_type, style):
            ...
            try:
                ...
                if not stored_data:
                    error_content = html.Div([
                        ...
                        html.P("Please upload data first to generate health reports.")
                    ])
                ...
                report_data = {
                    ...
                    "executive_summary": {
                        ...
                        "data_quality": "Good",
                        "key_findings": ["Signal analysis completed", "Data quality acceptable"]
                    },
                    "vital_signs": {
                    ...
                    "recommendations": {
                        "recommendations": ["Continue monitoring", "All values within normal range"],
                        "priority": "Low"
                    }
                }
                ...
                preview = html.Div([
                    html.H4(f"📋 Health Report Preview - {report_type.replace('_', ' ').title()}"),
                    html.P(f"Style: {style.title()}", className="text-muted"),
                    html.Hr(),
                    html.H5("📊 Executive Summary"),
                    html.P(f"Signal Duration: {report_data['executive_summary']['signal_duration']}"),
                    html.P(f"Data Quality: {report_data['executive_summary']['data_quality']}")
                ])
                ...
                content = html.Div([
                    ...
                    html.P(f"Signal Duration: {report_data['executive_summary']['signal_duration']}"),
                    html.H5("💓 Vital Signs"),
                    html.P(f"Heart Rate: {report_data['vital_signs']['heart_rate']['value']} {report_data['vital_signs']['heart_rate']['unit']} - {report_data['vital_signs']['heart_rate']['status']}"),
                    html.P(f"Blood Pressure: {report_data['vital_signs']['blood_pressure']['systolic']}/{report_data['vital_signs']['blood_pressure']['diastolic']} {report_data['vital_signs']['blood_pressure']['unit']} - {report_data['vital_signs']['blood_pressure']['status']}"),
                    html.H5("💡 Recommendations"),
                    html.Ul([html.Li(rec) for rec in report_data['recommendations']['recommendations']])
                ])
                ...
                templates = html.Div([
                    ...
                    html.Div([
                        html.Div([
                            ...
                            html.P("Full analysis with all sections", className="text-muted small"),
                            dbc.Badge("Selected" if report_type == "comprehensive" else "Available", 
                                     color="success" if report_type == "comprehensive" else "secondary", className="mt-2")
                        ], className="col-md-6 mb-3"),
                        html.Div([
                            html.H6("Clinical Summary"),
                            html.P("Essential clinical information", className="text-muted small"),
                            dbc.Badge("Selected" if report_type == "clinical" else "Available", 
                                     color="success" if report_type == "clinical" else "secondary", className="mt-2")
                        ], className="col-md-6 mb-3")
                    ], className="row")
                ])
                ...
                template_settings = html.Div([
                    ...
                    html.Table([
                        ...
                        html.Tbody([
                            html.Tr([html.Td("Report Type"), html.Td(report_type.replace("_", " ").title())]),
                            html.Tr([html.Td("Style"), html.Td(style.title())]),
                            html.Tr([html.Td("Format"), html.Td("PDF (Default)")])
                        ])
                ...
                history = html.Div([
                    ...
                    html.Table([
                        html.Thead(html.Tr([html.Th("Date"), html.Th("Type"), html.Th("Status")])),
                        html.Tbody([
                            html.Tr([html.Td("2024-01-15"), html.Td("Comprehensive"), html.Td(dbc.Badge("Generated", color="success"))]),
                            html.Tr([html.Td("2024-01-10"), html.Td("Clinical Summary"), html.Td(dbc.Badge("Generated", color="success"))])
                        ])
                    ], className="table table-sm")
                ])
                ...
                return (preview, content, templates, template_settings, history,
                        stored_report_data, stored_config)
