...
def register_features_callbacks(app):
    ...
    def features_analysis_callback(n_clicks, pathname, signal_type, preprocessing, categories, advanced_options):
        ...
        try:
            ...
            signal_quality = _extract_signal_quality_metrics(processed_signal, sampling_freq)
            physiological_features = _extract_physiological_features(processed_signal, sampling_freq, signal_type)
            ...
            logger.info(f"Creating plots with signal data length: {len(processed_signal)}, categories: {categories}")
...
def _import_vitaldsp_modules():
    ...
    try:
        ...
        logger.warning(f"vitalDSP modules not available: {e}, using scipy/numpy fallback")
        return False
...
def extract_comprehensive_features(signal_data, sampling_freq, categories, advanced_options):
    ...
    try:
        ...
        logger.info(f"Signal data length: {len(signal_data)}, sampling_freq: {sampling_freq}")
        ...
        if "spectral" in categories:
            features["spectral"] = extract_spectral_features(signal_data, sampling_freq, vitaldsp_available)
        
        # Temporal Features
        if "temporal" in categories:
            features["temporal"] = extract_temporal_features(signal_data, sampling_freq, vitaldsp_available)
        
        # Morphological Features
        if "morphological" in categories:
            features["morphological"] = extract_morphological_features(signal_data, sampling_freq, vitaldsp_available)
        
        # Entropy Features
        if "entropy" in categories:
            features["entropy"] = extract_entropy_features(signal_data, vitaldsp_available)
        
        # Fractal Features
        if "fractal" in categories:
            features["fractal"] = extract_fractal_features(signal_data, vitaldsp_available)
        
        # Advanced Features
        if advanced_options:
            features["advanced"] = extract_advanced_features(signal_data, sampling_freq, advanced_options, vitaldsp_available)
        ...
        if vitaldsp_available:
            try:
                ...
                try:
                    ...
                    stft_available = True
                    stft_shape = stft_result.shape if hasattr(stft_result, 'shape') else "N/A"
                except Exception as e:
                ...
                logger.warning(f"vitalDSP spectral features failed, falling back to scipy: {e}")
        ...
        if vitaldsp_available:
            try:
                ...
                if len(peaks) > 1:
                    ...
                    intervals = np.diff(peaks) / sampling_freq * 1000  # Convert to milliseconds
                    tdf = TimeDomainFeatures(intervals)
                    ...
                    temporal_features = {
                        ...
                        "num_peaks": len(peaks),
                        "mean_interval": np.mean(intervals) / 1000,  # Convert back to seconds
                ...
                logger.warning(f"vitalDSP temporal features failed, falling back to scipy: {e}")
                vitaldsp_available = False
...
def extract_morphological_features(signal_data, sampling_freq, vitaldsp_available=False):
    ...
    try:
        if vitaldsp_available:
            try:
                ...
                from vitalDSP.physiological_features.waveform import WaveformMorphology
                wm = WaveformMorphology(signal_data, sampling_freq)
                
                # Find peaks and valleys using vitalDSP
                peaks, _ = signal.find_peaks(signal_data, height=np.mean(signal_data) + np.std(signal_data), 
                                           distance=int(sampling_freq * 0.3))
                valleys, _ = signal.find_peaks(-signal_data, height=np.mean(-signal_data) + np.std(-signal_data), 
                                             distance=int(sampling_freq * 0.3))
                ...
                logger.warning(f"vitalDSP morphological features failed, falling back to scipy: {e}")
                vitaldsp_available = False
    ...
    try:
        if vitaldsp_available:
            try:
                ...
                logger.warning(f"vitalDSP entropy features failed, falling back to scipy: {e}")
                vitaldsp_available = False
    ...
    try:
        if vitaldsp_available:
            try:
                ...
                logger.warning(f"vitalDSP fractal features failed, falling back to scipy: {e}")
                vitaldsp_available = False
...
def extract_advanced_features(signal_data, sampling_freq, advanced_options, vitaldsp_available=False):
            ...
            if option == "cross_correlation":
                if vitaldsp_available:
                    try:
                        ...
                        from vitalDSP.physiological_features.cross_correlation import CrossCorrelationFeatures
                        cc = CrossCorrelationFeatures(signal_data, signal_data, sampling_freq)
                        cross_corr, lag = cc.compute_cross_correlation()
                        
                        advanced_features["cross_correlation"] = {
                            "cross_correlation": cross_corr.tolist() if hasattr(cross_corr, 'tolist') else cross_corr,
                            "max_lag": lag,
                            "correlation_strength": np.max(np.abs(cross_corr)) if hasattr(cross_corr, '__len__') else 0,
                            "vitaldsp_used": True
                        }
                    except Exception as e:
                        logger.warning(f"vitalDSP cross-correlation failed, falling back to scipy: {e}")
                        # Fallback to scipy
                ...
                if vitaldsp_available:
                    try:
                        ...
                        advanced_features["phase_analysis"] = {
                            ...
                            "analytic_signal": analytic_signal.tolist() if hasattr(analytic_signal, 'tolist') else analytic_signal,
                            "vitaldsp_used": True
                        }
                    except Exception as e:
                        logger.warning(f"vitalDSP phase analysis failed, falling back to scipy: {e}")
                        # Fallback to scipy
                ...
                if vitaldsp_available:
                    try:
                        ...
                        advanced_features["wavelet"] = {"error": f"Wavelet transform failed: {str(e)}"}
                ...
                if vitaldsp_available:
                    try:
                        ...
                        advanced_features["ml_features"] = {
                            "anomalies_detected": len(anomalies) if hasattr(anomalies, '__len__') else 0,
                            "anomaly_indices": anomalies.tolist() if hasattr(anomalies, 'tolist') else anomalies,
                            "vitaldsp_used": True
                        }
                    except Exception as e:
                        logger.warning(f"vitalDSP ML features failed: {e}")
                        advanced_features["ml_features"] = {"error": f"ML features failed: {str(e)}"}
...
def create_comprehensive_features_display(features, signal_type, categories):
    ...
    try:
        ...
        header_section = html.Div([
            html.H4(f"🚀 Advanced Feature Analysis Results", className="text-primary mb-3"),
            html.Div([
                html.Span(f"Signal Type: {signal_type.upper()}", className="badge bg-primary me-2"),
                html.Span(f"vitalDSP: {'✅ Used' if vitaldsp_used else '⚠️ Fallback'}", 
                         className=f"badge {'bg-success' if vitaldsp_used else 'bg-warning'} me-2"),
                html.Span(f"Categories: {len(categories)}", className="badge bg-info")
            ], className="mb-3")
        ])
        ...
        if "statistical" in categories and "error" not in features.get("statistical", {}):
            ...
            stats_section = html.Div([
                ...
                html.Div([
                    html.Div([
                        html.H6("Basic Statistics", className="text-primary"),
                        html.P(f"Mean: {stats.get('mean', 0):.4f}", className="mb-1"),
                        html.P(f"Standard Deviation: {stats.get('std', 0):.4f}", className="mb-1"),
                        html.P(f"Variance: {stats.get('variance', 0):.4f}", className="mb-1"),
                        html.P(f"RMS: {stats.get('rms', 0):.4f}", className="mb-1")
                    ], className="col-md-6"),
                    html.Div([
                        html.H6("Shape Statistics", className="text-primary"),
                        html.P(f"Skewness: {stats.get('skewness', 0):.4f}", className="mb-1"),
                        html.P(f"Kurtosis: {stats.get('kurtosis', 0):.4f}", className="mb-1"),
                        html.P(f"Peak-to-Peak: {stats.get('peak_to_peak', 0):.4f}", className="mb-1"),
                        html.P(f"Crest Factor: {stats.get('crest_factor', 0):.4f}", className="mb-1")
                    ], className="col-md-6")
                ], className="row")
            ], className="mb-4")
        ...
        if "spectral" in categories and "error" not in features.get("spectral", {}):
            ...
            spec_section = html.Div([
                ...
                html.Div([
                    html.Div([
                        html.H6("Frequency Analysis", className="text-primary"),
                        html.P(f"Spectral Centroid: {spec.get('spectral_centroid', 0):.2f} Hz", className="mb-1"),
                        html.P(f"Spectral Bandwidth: {spec.get('spectral_bandwidth', 0):.2f} Hz", className="mb-1"),
                        html.P(f"Dominant Frequency: {spec.get('dominant_frequency', 0):.2f} Hz", className="mb-1")
                    ], className="col-md-6"),
                    html.Div([
                        html.H6("Energy Analysis", className="text-primary"),
                        html.P(f"Total Energy: {spec.get('total_energy', 0):.2f}", className="mb-1"),
                        html.P(f"Frequency Range: {spec.get('frequency_range', 'N/A')}", className="mb-1"),
                        html.P(f"STFT Available: {'✅' if spec.get('stft_available', False) else '❌'}", className="mb-1")
                    ], className="col-md-6")
                ], className="row")
            ], className="mb-4")
        ...
        if "temporal" in categories and "error" not in features.get("temporal", {}):
            ...
            temp_section = html.Div([
                ...
                html.Div([
                    html.Div([
                        html.H6("Signal Properties", className="text-primary"),
                        html.P(f"Signal Duration: {temp.get('signal_duration', 0):.2f} seconds", className="mb-1"),
                        html.P(f"Sampling Frequency: {temp.get('sampling_frequency', 0)} Hz", className="mb-1"),
                        html.P(f"Number of Samples: {temp.get('num_samples', 0)}", className="mb-1")
                    ], className="col-md-6"),
                    html.Div([
                        html.H6("Peak Analysis", className="text-primary") if "num_peaks" in temp else html.Div(),
                        html.P(f"Number of Peaks: {temp.get('num_peaks', 0)}", className="mb-1") if "num_peaks" in temp else html.Div(),
                        html.P(f"Mean Interval: {temp.get('mean_interval', 0):.3f} seconds", className="mb-1") if "mean_interval" in temp else html.Div(),
                        html.P(f"Interval Std: {temp.get('std_interval', 0):.3f} seconds", className="mb-1") if "std_interval" in temp else html.Div()
                    ], className="col-md-6")
                ], className="row")
            ], className="mb-4")
            ...
            morph_section = html.Div([
                ...
                html.Div([
                    html.Div([
                        ...
                        html.P(f"Number of Peaks: {morph.get('num_peaks', 0)}", className="mb-1"),
                        html.P(f"Number of Valleys: {morph.get('num_valleys', 0)}", className="mb-1")
                    ], className="col-md-6"),
                    html.Div([
                        html.H6("Peak Statistics", className="text-primary") if "mean_peak_height" in morph else html.Div(),
                        html.P(f"Mean Peak Height: {morph.get('mean_peak_height', 0):.4f}", className="mb-1") if "mean_peak_height" in morph else html.Div(),
                        html.P(f"Peak Height Std: {morph.get('std_peak_height', 0):.4f}", className="mb-1") if "std_peak_height" in morph else html.Div(),
                        html.P(f"Max Peak Height: {morph.get('max_peak_height', 0):.4f}", className="mb-1") if "max_peak_height" in morph else html.Div()
                    ], className="col-md-6")
                ], className="row")
            ], className="mb-4")
        ...
        if "entropy" in categories and "error" not in features.get("entropy", {}):
            ...
            ent_section = html.Div([
                html.H5("🧠 Entropy & Complexity Features", className="text-secondary"),
                html.Div([
                    html.Div([
                        html.H6("Entropy Measures", className="text-primary"),
                        html.P(f"Sample Entropy: {ent.get('sample_entropy', 0):.4f}", className="mb-1"),
                        html.P(f"Approximate Entropy: {ent.get('approximate_entropy', 0):.4f}", className="mb-1"),
                        html.P(f"Fractal Dimension: {ent.get('fractal_dimension', 'N/A')}", className="mb-1")
                    ], className="col-md-6"),
                    html.Div([
                        html.H6("Advanced Measures", className="text-primary"),
                        html.P(f"Lyapunov Exponent: {ent.get('lyapunov_exponent', 'N/A')}", className="mb-1"),
                        html.P(f"DFA Result: {ent.get('dfa_result', 'N/A')}", className="mb-1"),
                        html.P(f"Poincaré Features: {'Available' if ent.get('poincare_features') else 'N/A'}", className="mb-1")
                    ], className="col-md-6")
                ], className="row"),
                html.Div([
                    html.Small(f"vitalDSP: {'✅ Used' if ent.get('vitaldsp_used', False) else '⚠️ Fallback'}", 
        ...
        if "advanced" in features and "error" not in features.get("advanced", {}):
            ...
            adv_section = html.Div([
                ...
                html.Div([
                    html.Div([
                        html.H6("Cross-Correlation", className="text-primary") if "cross_correlation" in adv else html.Div(),
                        html.P(f"Max Lag: {adv.get('cross_correlation', {}).get('max_lag', 'N/A')}", className="mb-1") if "cross_correlation" in adv else html.Div(),
                        html.P(f"Correlation Strength: {adv.get('cross_correlation', {}).get('correlation_strength', 'N/A'):.4f}", className="mb-1") if "cross_correlation" in adv else html.Div(),
                        html.P(f"vitalDSP: {'✅' if adv.get('cross_correlation', {}).get('vitaldsp_used', False) else '⚠️'}", className="mb-1") if "cross_correlation" in adv else html.Div()
                    ], className="col-md-6") if "cross_correlation" in adv else html.Div(),
                    html.Div([
                        html.H6("Phase Analysis", className="text-primary") if "phase_analysis" in adv else html.Div(),
                        html.P(f"Phase Range: {adv.get('phase_analysis', {}).get('phase_range', 'N/A'):.4f}", className="mb-1") if "phase_analysis" in adv else html.Div(),
                        html.P(f"Phase Std: {adv.get('phase_analysis', {}).get('phase_std', 'N/A'):.4f}", className="mb-1") if "phase_analysis" in adv else html.Div(),
                        html.P(f"vitalDSP: {'✅' if adv.get('phase_analysis', {}).get('vitaldsp_used', False) else '⚠️'}", className="mb-1") if "phase_analysis" in adv else html.Div()
                    ], className="col-md-6") if "phase_analysis" in adv else html.Div()
                ], className="row")
            ], className="mb-4")
            sections.append(adv_section)
        
        # Signal Quality Features
        if "signal_quality" in features and "error" not in features.get("signal_quality", {}):
            sq = features["signal_quality"]
            sq_section = html.Div([
                html.H5("🔍 Signal Quality Assessment", className="text-success"),
                html.Div([
                    html.Div([
                        html.H6("Quality Metrics", className="text-primary"),
                        html.P(f"Signal-to-Noise Ratio: {sq.get('snr', 0):.2f} dB", className="mb-1"),
                        html.P(f"Baseline Wander: {sq.get('baseline_wander', 0):.4f}", className="mb-1"),
                        html.P(f"Motion Artifacts: {sq.get('motion_artifacts', 0)}", className="mb-1")
                    ], className="col-md-6"),
                    html.Div([
                        html.H6("Quality Assessment", className="text-primary"),
                        html.P(f"vitalDSP: {'✅ Used' if sq.get('vitaldsp_used', False) else '⚠️ Fallback'}", className="mb-1"),
                        html.P(f"Quality Score: {'Good' if sq.get('snr', 0) > 10 else 'Fair' if sq.get('snr', 0) > 5 else 'Poor'}", className="mb-1")
                    ], className="col-md-6")
                ], className="row")
            ], className="mb-4")
        ...
        if "physiological" in features and "error" not in features.get("physiological", {}):
            ...
            if phys.get("vitaldsp_used", False) and "waveform_features" in phys:
                phys_section = html.Div([
                    ...
                    html.Div([
                        html.H6("Waveform Morphology", className="text-primary"),
                        html.P(f"vitalDSP: ✅ Used", className="mb-1"),
                        html.P(f"Features Available: {len(phys.get('waveform_features', {}))}", className="mb-1"),
                        html.Small("Advanced physiological waveform analysis completed", className="text-muted")
                    ])
                ], className="mb-4")
        ...
        summary_section = html.Div([
            ...
            html.P(f"Successfully extracted features from {len(categories)} categories"),
            html.P(f"Signal type: {signal_type}"),
            html.P(f"vitalDSP utilization: {'High' if vitaldsp_used else 'Limited'}"),
            html.Small("Features extracted using advanced signal processing algorithms", className="text-muted")
        ], className="mt-4")
...
def create_features_analysis_plots(signal_data, features, categories, sampling_freq):
    ...
    try:
        logger.info(f"Plotting function called with signal_data length: {len(signal_data)}, categories: {categories}")
        ...
        signal_quality_plot = 1 if "signal_quality" in features and "error" not in features.get("signal_quality", {}) else 0
        ...
        subplot_titles = ([f"{cat.title()} Features" for cat in categories] + 
                         ["Cross-Correlation", "Phase Analysis", "Wavelet Analysis"][:advanced_plots] +
...
def _extract_signal_quality_metrics(signal_data, sampling_freq):
    ...
    try:
        ...
        motion_artifacts = np.sum(np.abs(np.diff(signal_data)) > 2 * np.std(signal_data))
        ...
        return {
            "snr": (np.std(signal_data) / np.mean(np.abs(np.diff(signal_data)))) if np.mean(np.abs(np.diff(signal_data))) > 0 else 0,
            "baseline_wander": np.std(signal_data - np.mean(signal_data)),
            "motion_artifacts": np.sum(np.abs(np.diff(signal_data)) > 2 * np.std(signal_data)),
            "vitaldsp_used": False
        }
...
def _extract_physiological_features(signal_data, sampling_freq, signal_type):
    ...
    try:
        if signal_type.lower() in ['ppg', 'ecg']:
            ...
            from vitalDSP.physiological_features.waveform import WaveformMorphology
            wm = WaveformMorphology(signal_data, sampling_freq, signal_type=signal_type.upper())
            ...
            try:
                ...
                if signal_type.lower() == 'ecg':
                    ...
                    if len(peaks) > 0:
                        q_valleys = wm.detect_q_valley(peaks) if hasattr(wm, 'detect_q_valley') else []
                        s_valleys = wm.detect_s_valley(peaks) if hasattr(wm, 'detect_s_valley') else []
                        
                        features = {
                            "r_peaks": len(peaks),
                            "q_valleys": len(q_valleys),
                            "s_valleys": len(s_valleys),
                            "heart_rate": len(peaks) * 60 / (len(signal_data) / sampling_freq)
                        }
                    else:
                        features = {"r_peaks": 0, "q_valleys": 0, "s_valleys": 0, "heart_rate": 0}
                else:
                    # For PPG, get pulse features
                    if len(peaks) > 0:
                        troughs = wm.detect_troughs(peaks) if hasattr(wm, 'detect_troughs') else []
                        
                        features = {
                            "systolic_peaks": len(peaks),
                            "troughs": len(troughs),
                            "pulse_rate": len(peaks) * 60 / (len(signal_data) / sampling_freq)
                        }
                    else:
