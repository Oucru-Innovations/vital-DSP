...
def register_upload_callbacks(app):
    ...
    def handle_all_uploads(upload_contents, load_path_clicks, load_sample_clicks, 
        ...
        file_path_loading_style = {"display": "block", "animation": "fadeInUp 0.4s ease-out"}
        ...
        try:
            ...
            if trigger_id == "upload-data" and upload_contents:
                ...
                if filename.endswith('.csv'):
                    ...
                    return "❌ Unsupported file format. Please upload CSV or TXT files.", no_update, no_update, no_update, no_update, no_update, no_update, no_update, no_update, no_update, no_update, no_update, False, False, no_update, {"display": "none"}, "upload-area"
                ...
                if file_path.endswith('.csv'):
                    ...
                    return "❌ Unsupported file format. Please use CSV or TXT files.", no_update, no_update, no_update, no_update, no_update, no_update, no_update, no_update, no_update, no_update, no_update, False, False, no_update, {"display": "none"}, "upload-area"
            ...
            return (status, df.to_dict('records'), data_info, preview, 
                   ...
                   progress_bar, progress_style, buttons_disabled, buttons_disabled, 
                   file_path_loading, file_path_loading_style, upload_area_class)
            ...
            file_path_loading_style = {"display": "none"}
            upload_area_class = "upload-area"
            return f"❌ Error loading data: {str(e)}", no_update, no_update, no_update, no_update, no_update, no_update, no_update, no_update, no_update, progress_bar, progress_style, buttons_disabled, buttons_disabled, file_path_loading, file_path_loading_style, upload_area_class
    ...
    def auto_detect_columns(n_clicks, uploaded_data, data_config):
        ...
        try:
            ...
            return no_update, no_update, no_update, no_update, no_update, button_disabled, button_content
    ...
    def process_data_with_columns(n_clicks, time_col, signal_col, red_col, ir_col, 
        ...
        progress_section = create_processing_progress_section()
        progress_style = {"display": "block", "animation": "slideInDown 0.5s ease-out"}
        
        # Create initial processing progress section (Step 1: Validating)
        test_progress = html.Div([
            html.Div([
                html.I(className="fas fa-cogs fa-spin me-3 text-primary", style={"fontSize": "2rem"}),
            ...
            html.Div([
                html.Div([
                    html.I(className="fas fa-spinner fa-spin text-primary me-3", style={"fontSize": "1.2rem"}),
                    html.Span("Validating column mapping...", className="fw-semibold fs-5 text-primary")
                ], className="mb-3 progress-step active"),
                
                html.Div([
                    html.I(className="fas fa-clock text-muted me-3", style={"fontSize": "1.2rem"}),
                    html.Span("Processing signal data", className="fw-semibold fs-5 text-muted")
                ], className="mb-3 progress-step pending"),
                
                html.Div([
                    html.I(className="fas fa-clock text-muted me-3", style={"fontSize": "1.2rem"}),
                    html.Span("Storing processed data", className="fw-semibold fs-5 text-muted")
                ], className="mb-3 progress-step pending")
            ], className="mb-4"),
            
            html.Div([
                html.Strong("Step 1 of 3: Validating column mapping", className="text-primary fs-5"),
                html.Br(),
                html.Small("Checking column selections and data format", className="text-muted")
        ...
        ], className="p-4 bg-light border border-primary shadow", style={
            "zIndex": 1000,
            "position": "relative",
            "minHeight": "250px",
            "fontSize": "1.1rem"
        ...
        try:
            ...
            completed_progress = html.Div([
                html.Div([
                    html.I(className="fas fa-check-circle text-success me-3", style={"fontSize": "2rem"}),
                    html.H3("Processing Complete!", className="text-success mb-0")
                ], className="d-flex align-items-center justify-content-center mb-4"),
                ...
                html.Div([
                    html.Div([
                        html.I(className="fas fa-check-circle text-success me-3", style={"fontSize": "1.2rem"}),
                        html.Span("Column mapping validated", className="fw-semibold fs-5 text-success")
                    ], className="mb-3 progress-step completed"),
                    
                    html.Div([
                        html.I(className="fas fa-check-circle text-success me-3", style={"fontSize": "1.2rem"}),
                        html.Span("Signal data processed", className="fw-semibold fs-5 text-success")
                    ], className="mb-3 progress-step completed"),
                    
                    html.Div([
                        html.I(className="fas fa-check-circle text-success me-3", style={"fontSize": "1.2rem"}),
                        html.Span("Data stored successfully", className="fw-semibold fs-5 text-success")
                    ], className="mb-3 progress-step completed")
                ], className="mb-4"),
                
                html.Div([
                    html.Strong("All steps completed successfully!", className="text-success fs-5"),
                    html.Br(),
            ...
            ], className="p-4 bg-light border border-success shadow", style={
                "zIndex": 1000,
                "position": "relative",
                "minHeight": "250px",
                "fontSize": "1.1rem"
            ...
            return status, df.to_dict('records'), data_config, preview, completed_progress, progress_style, process_button_disabled, process_button_content
            ...
            error_progress = html.Div([
                html.Div([
                    html.I(className="fas fa-exclamation-triangle text-danger me-3", style={"fontSize": "2rem"}),
                ...
                html.Div([
                    ...
                    html.Div([
                        html.I(className="fas fa-times-circle text-danger me-3", style={"fontSize": "1.2rem"}),
                        html.Span("Processing failed", className="fw-semibold fs-5 text-danger")
                    ], className="mb-3 progress-step error"),
                    
                    html.Div([
                        html.I(className="fas fa-clock text-muted me-3", style={"fontSize": "1.2rem"}),
                        html.Span("Data storage skipped", className="fw-semibold fs-5 text-muted")
                    ], className="mb-3 progress-step pending")
                ], className="mb-4"),
                
                html.Div([
                    html.Strong("Error occurred during processing", className="text-danger fs-5"),
                    html.Br(),
            ...
            ], className="p-4 bg-light border border-danger shadow", style={
                "zIndex": 1000,
                "position": "relative",
                "minHeight": "250px",
                "fontSize": "1.1rem"
            ...
            return f"❌ Error processing data: {str(e)}", no_update, no_update, no_update, error_progress, progress_style, process_button_disabled, process_button_content
...
def create_file_path_loading_indicator():
    ...
    return html.Div([
        dbc.Alert([
            html.Div([
                html.I(className="fas fa-spinner fa-spin me-2 text-primary", style={"fontSize": "1.2rem"}),
                html.Span("Loading file from path...", className="fw-semibold text-primary")
        ...
        ], color="primary", className="border-0 mb-0", style={"backgroundColor": "rgba(13, 110, 253, 0.1)"})
    ])
...
def create_upload_progress_bar():
    ...
    return html.Div([
        dbc.Alert([
            html.Div([
                html.I(className="fas fa-upload fa-spin me-3 text-primary", style={"fontSize": "1.5rem"}),
                html.Span("Uploading data...", className="fw-bold text-primary", style={"fontSize": "1.1rem"})
            ...
            html.Div([
                html.Div([
                    ...
                    html.Span("File selected", className="fw-semibold text-success")
                ], className="mb-2 progress-step completed"),
                html.Div([
                    html.I(className="fas fa-spinner fa-spin text-primary me-2"),
                    html.Span("Reading file contents", className="fw-semibold text-primary")
                ], className="mb-2 progress-step active"),
                html.Div([
                    html.I(className="fas fa-clock text-muted me-2"),
                    html.Span("Processing data", className="fw-semibold text-muted")
                ], className="mb-2 progress-step pending")
            ])
        ], color="primary", className="border-0", style={"backgroundColor": "rgba(13, 110, 253, 0.1)"})
    ])
...
def create_processing_progress_section():
    ...
    return html.Div([
        dbc.Alert([
            html.Div([
                html.I(className="fas fa-cogs fa-spin me-3 text-info", style={"fontSize": "1.5rem"}),
                html.Span("Processing Data", className="fw-bold text-info", style={"fontSize": "1.1rem"})
            ], className="mb-3 text-center"),
            html.Div([
                html.Div([
                    html.I(className="fas fa-check-circle text-success me-2"),
                    html.Span("Validating column mapping", className="fw-semibold")
                ], className="mb-2 progress-step completed"),
                html.Div([
                    html.I(className="fas fa-spinner fa-spin text-primary me-2"),
                    html.Span("Processing signal data", className="fw-semibold text-primary")
                ], className="mb-2 progress-step active"),
                html.Div([
                    html.I(className="fas fa-clock text-muted me-2"),
                    html.Span("Storing processed data", className="fw-semibold text-muted")
                ], className="mb-2 progress-step pending")
            ]),
            dbc.Progress(
                value=66,
            ...
            html.Div([
                html.Strong("Step 2 of 3: Processing signal data", className="text-info"),
                html.Br(),
                html.Small("This may take a few moments depending on data size", className="text-muted")
            ], className="text-center")
        ], color="info", className="border-0", style={"backgroundColor": "rgba(23, 162, 184, 0.1)"})
    ])
