...
class ChunkedDataService:
    ...
    def __init__(self, max_cache_size: int = 100, max_memory_mb: int = 200):
        ...
        logger.info(f"ChunkedDataService initialized with {max_cache_size} cache size, {max_memory_mb}MB memory limit")
        ...
        try:
            ...
            if progress_callback and task_id:
                progress = LoadingProgress(
                    task_id=task_id,
                    progress_percent=min(100.0, (chunk_index + 1) * 10),  # Rough estimate
    ...
    def _load_chunk_pandas(self, file_path: str, chunk_index: int, chunk_size: int) -> pd.DataFrame:
...
class MemoryMappedDataService:
    ...
    def __init__(self, max_memory_mb: int = 500):
        ...
        logger.info(f"MemoryMappedDataService initialized with {max_memory_mb}MB memory limit")
    ...
    def get_segment(
        ...
        with self._lock:
            ...
            try:
                ...
                logger.debug(f"Accessed segment {start_idx}-{end_idx} from map {map_id}")
                return segment
        ...
        try:
            if 'preview_size' in request:
                ...
                if file_size > 500 * 1024 * 1024:  # > 500MB
                    ...
                    # Extract segment
                    segment_data = chunk.iloc[start_idx:end_idx] if isinstance(chunk, pd.DataFrame) else chunk[start_idx:end_idx]
            ...
            self.stats["average_load_time"] = (
                (self.stats["average_load_time"] * (self.stats["background_loads"] - 1) + load_time) 
...
class EnhancedDataService:
    ...
    def __init__(self, max_memory_mb: int = 500):
        ...
        self.memory_mapped_service = MemoryMappedDataService(max_memory_mb=max_memory_mb // 2)
...
if __name__ == "__main__":
    ...
    print("  - EnhancedDataService: Unified interface with automatic strategy selection")
