...
from typing import Dict, List, Any, Optional, Union, Callable, Generator, Tuple, Iterator
...
class MemoryEfficientCache:
    ...
    def _compress_chunk(self, chunk: LazyChunk) -> LazyChunk:
        ...
        try:
            if chunk.data is not None:
                ...
                self.stats["memory_saved"] += chunk.memory_size - self._calculate_chunk_size(chunk)
    ...
    def _evict_if_needed(self, new_chunk_size: int) -> None:
        ...
        current_memory = sum(self._calculate_chunk_size(chunk) for chunk in self.cache.values())
...
class LazyChunkManager:
    ...
    def __init__(self, chunk_size_mb: int = 50, max_workers: int = 4):
        ...
        logger.info(f"LazyChunkManager initialized with {chunk_size_mb}MB chunks, {max_workers} workers")
    ...
    def create_chunks(
        ...
        if self.chunk_size_samples is None:
            ...
            self.chunk_size_samples = max(1000, min(self.chunk_size_samples, signal_length // 4))
        
        chunks = []
        total_chunks = (signal_length + self.chunk_size_samples - 1) // self.chunk_size_samples
    ...
    def _load_chunk(
        ...
        try:
            ...
            self.stats["average_load_time"] = (
                (self.stats["average_load_time"] * (self.stats["chunks_loaded"] - 1) + chunk.loading_time) /
    ...
    def prefetch_chunks(
        ...
        priority: LoadingPriority = LoadingPriority.LOW
    ) -> None:
        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                ...
                if progress_callback:
                    progress_callback(progress, f"Filtered chunk {i + 1}/{total_chunks}")
    ...
    def _apply_filter_to_chunk(
        ...
        try:
            ...
            self.stats["average_filtering_time"] = (
                (self.stats["average_filtering_time"] * (self.stats["filters_applied"] - 1) + chunk.filtering_time) /
...
class WebSocketProgressBroadcaster:
    ...
    def __init__(self, websocket_manager=None):
        ...
        """
        self.websocket_manager = websocket_manager or get_websocket_manager() if HEAVY_DATA_AVAILABLE else None
...
class ProgressiveDataLoader:
    ...
    def __init__(self, max_memory_mb: int = 500, chunk_size_mb: int = 50):
        ...
        logger.info(f"ProgressiveDataLoader initialized with {max_memory_mb}MB memory limit")
        ...
        try:
            ...
            for result in self.filter_processor.process_streaming(
                ...
                self.progress_broadcaster.update_progress(
                    request_id, completed_chunks, f"Processed chunk {completed_chunks}/{len(chunks)}"
                )
            ...
            logger.info(f"Completed lazy filtering request {request_id} in {processing_time:.3f}s")
