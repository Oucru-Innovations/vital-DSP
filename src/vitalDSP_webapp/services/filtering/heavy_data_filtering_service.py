...
class IntelligentStrategySelector:
    ...
    def select_strategy(
        ...
        if user_preference and self._is_strategy_valid(user_preference, data_size_mb, available_memory_mb):
        ...
        logger.info(f"Selected strategy: {strategy.value}, mode: {mode.value} for {data_size_mb:.1f}MB data")
    ...
    def _is_strategy_valid(
        ...
        if strategy == FilteringStrategy.STANDARD:
            return data_size_mb < self.thresholds["standard_max"] and data_size_mb < available_memory_mb * 0.5
...
class LazyLoadingFilteringService:
    ...
    def __init__(self, chunk_size_mb: int = 50, max_concurrent_chunks: int = 4):
        ...
        logger.info(f"LazyLoadingFilteringService initialized with {chunk_size_mb}MB chunks")
    ...
    def process_lazy(
        ...
        try:
            ...
            signal_length = len(request.signal_data) if request.signal_data is not None else 0
            chunk_size_samples = int(self.chunk_size_mb * 1024 * 1024 / (signal_length * 8))  # Assuming float64
            chunk_size_samples = max(1000, min(chunk_size_samples, signal_length // 4))  # Reasonable bounds
            
            total_chunks = (signal_length + chunk_size_samples - 1) // chunk_size_samples
            
            logger.info(f"Processing {signal_length} samples in {total_chunks} chunks of {chunk_size_samples} samples")
    ...
    def _filter_chunk(self, chunk: LazyLoadingChunk, filter_params: Dict[str, Any]) -> np.ndarray:
...
class HeavyDataFilteringService:
    ...
    def __init__(self, max_memory_mb: int = 1000):
        ...
        logger.info(f"HeavyDataFilteringService initialized with {max_memory_mb}MB memory limit")
    ...
    def process_filtering_request(
        ...
        try:
            ...
            logger.info(f"Processing request {request.request_id} with strategy: {strategy.value}, mode: {mode.value}")
            ...
            logger.info(f"Completed request {request.request_id} in {processing_time:.3f}s using {strategy.value}")
            return result
    ...
    def _process_standard(
        ...
        try:
            if not VITALDSP_AVAILABLE or request.signal_data is None:
                ...
                filtered_signal = self._apply_basic_filter(request.signal_data, request.filter_params)
            else:
    ...
    def _process_memory_mapped(
        ...
        try:
            if not VITALDSP_AVAILABLE or not request.file_path:
                raise ValueError("Memory-mapped processing requires file path and vitalDSP")
            ...
            total_samples = len(request.signal_data) if request.signal_data is not None else 0
            ...
            for start_idx in range(0, total_samples, segment_size):
                ...
                if progress_callback:
                    ...
                    progress_callback(progress, f"Processing segment {start_idx}-{end_idx}")
                
                # Load segment
                segment_data = request.signal_data[start_idx:end_idx]
                
                # Apply filtering
                filtered_segment = self._apply_basic_filter(segment_data, request.filter_params)
    ...
    def _process_chunked_background(self, request: FilteringRequest) -> List[FilteringResult]:
    ...
    def _apply_basic_filter(self, signal_data: np.ndarray, filter_params: Dict[str, Any]) -> np.ndarray:
