    ...
    def _update_task_redis(self, task: Task) -> bool:
        ...
        try:
            ...
            self.redis_client.hset(
                task_key, mapping={"progress": task.progress, "message": task.message}
            )
            self.stats["redis_operations"] += 1
            return True
        ...
        if self.use_redis:
            try:
                ...
                for priority in TaskPriority:
                    ...
                    stats["queue_sizes"][priority.name] = self.redis_client.llen(
                        queue_key
                    )
                ...
                for task_key in task_keys:
                    ...
                    if task_data:
                        ...
                        if task.created_at and task.timeout:
                            expiry_time = task.created_at + timedelta(
...
class TaskProcessor:
    ...
    def start(self) -> None:
        ...
        for i in range(self.max_workers):
            worker = threading.Thread(
                target=self._worker, daemon=True, name=f"TaskWorker-{i}"
            )
            worker.start()
    ...
    def _process_task(self, task: Task) -> None:
        ...
        try:
            ...
            (
                self.task_queue._update_task_redis(task)
                if self.task_queue.use_redis
                else self.task_queue.memory_queue.update_task(task)
            )
            ...
            if task.retry_count < task.max_retries:
                ...
                task.message = (
                    f"Retrying ({task.retry_count}/{task.max_retries}): {error_msg}"
                )
    ...
    def _execute_signal_processing(
        self, task: Task, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        self.task_queue.update_task_progress(
            task.id, 10.0, "Starting signal processing"
        )
        ...
        # Update progress
        self.task_queue.update_task_progress(
            task.id, 30.0, "Processing signal through pipeline"
        )
    ...
    def _execute_data_loading(
        ...
        # Update progress
        self.task_queue.update_task_progress(
            task.id, 20.0, f"Loading data from {file_path}"
        )
        ...
        try:
            ...
            # Update progress
            self.task_queue.update_task_progress(
                task.id, 80.0, "Data loaded successfully"
            )
    ...
    def _execute_quality_assessment(
        ...
        # Update progress
        self.task_queue.update_task_progress(
            task.id, 75.0, "Quality assessment completed"
        )
