...
class SettingsService:
    ...
    def export_settings(self, format_type: str = "json") -> Optional[str]:
        ...
        try:
            if format_type == "json":
                return json.dumps(self._settings_to_dict(), indent=2, ensure_ascii=False)
    ...
    def validate_settings(self) -> Dict[str, List[str]]:
        ...
        if self.settings.analysis.peak_threshold < 0.1 or self.settings.analysis.peak_threshold > 1.0:
        ...
        data_errors = []
        if self.settings.data.max_file_size < 10 or self.settings.data.max_file_size > 1000:
            data_errors.append("Max file size must be between 10 and 1000 MB")
        if self.settings.data.auto_save_interval < 1 or self.settings.data.auto_save_interval > 60:
        ...
        system_errors = []
        if self.settings.system.max_cpu_usage < 10 or self.settings.system.max_cpu_usage > 100:
            system_errors.append("Max CPU usage must be between 10% and 100%")
        if self.settings.system.memory_limit_gb < 1 or self.settings.system.memory_limit_gb > 32:
