...
def _method_card(title, plot_id, result_id):
    return dbc.Card(
        [
            dbc.CardHeader(
                dbc.Row(
                    [
                        ...
                        dbc.Col(html.Div(id=result_id, className="text-end fw-bold text-primary small"), width=4),
                    ],
                    align="center",
                )
            ),
            dbc.CardBody(
                dcc.Loading(
                    dcc.Graph(id=plot_id, style={"height": "180px"},
...
def respiratory_layout():
    ...
    return html.Div(
        [
                                    ...
                                    [
                                        ...
                                        dbc.Col(
                                            [
                                                ...
                                                dbc.ButtonGroup(
                                                    [
                                                        dbc.Button("⏮", id="resp-btn-nudge-m10", color="secondary", size="sm", title="-10%"),
                                                        dbc.Button("◀", id="resp-btn-nudge-m1", color="secondary", size="sm", title="-5%"),
                                                        dbc.Button("▶", id="resp-btn-nudge-p1", color="secondary", size="sm", title="+5%"),
                                                        dbc.Button("⏭", id="resp-btn-nudge-p10", color="secondary", size="sm", title="+10%"),
                                                    ],
                                                    className="w-100",
                                                ),
                                            ],
                                        ...
                                        dbc.Col(
                                            [
                                                html.Label("Min Breath (s)", className="form-label fw-semibold small mb-1"),
                                                dbc.Input(id="resp-min-breath-duration", type="number",
                                                          value=1.8, min=0.5, max=5.0, step=0.1, size="sm"),
                                            ],
                                            md=1,
                                        ),
                                        dbc.Col(
                                            [
                                                html.Label("Max Breath (s)", className="form-label fw-semibold small mb-1"),
                                                dbc.Input(id="resp-max-breath-duration", type="number",
                                                          value=6.0, min=2.0, max=20.0, step=0.5, size="sm"),
                                            ],
                                            md=1,
                                        ),
                                    ],
            ...
            dbc.Row(
                [
                    dbc.Col(
                        dbc.Card(
                            [
                                dbc.CardHeader(html.H6("Methods Comparison & Ensemble", className="mb-0")),
                                dbc.CardBody(
                                    dcc.Loading(
                                        html.Div(id="resp-analysis-results",
                                                 children=html.P("Click Run Analysis to compute all methods.",
            ...
            dbc.Row(
                [
                    dbc.Col(_method_card("Counting (Peak Detection RR)", "resp-plot-counting", "resp-result-counting"), md=6),
                    dbc.Col(_method_card("FFT-Based RR", "resp-plot-fft-based", "resp-result-fft-based"), md=6),
                ],
                className="mb-3",
            ),
            ...
            dbc.Row(
                [
                    dbc.Col(_method_card("Frequency Domain RR (Welch PSD)", "resp-plot-freq-domain", "resp-result-freq-domain"), md=6),
                    dbc.Col(_method_card("Time Domain RR (Autocorrelation)", "resp-plot-time-domain", "resp-result-time-domain"), md=6),
                ],
                className="mb-3",
            ),
            ...
            dbc.Row(
                [
                    dbc.Col(_method_card("Peak Interval Detection", "resp-plot-peaks", "resp-result-peaks"), md=6),
                    dbc.Col(_method_card("Zero-Crossing Detection", "resp-plot-zero-crossing", "resp-result-zero-crossing"), md=6),
                ],
                className="mb-3",
            ),
