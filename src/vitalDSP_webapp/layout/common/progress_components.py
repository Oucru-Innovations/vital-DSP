...
def create_progress_indicator(
    ...
    return html.Div([
        ...
        ) if show_percentage else None,
    ], style={"display": "none"}, id=f"{progress_id}-container")
...
def create_progress_overlay(
    ...
    ], id=overlay_id, style={"display": "none"})
...
def create_progress_card(
    ...
    return dbc.Card([
        dbc.CardHeader([
            ...
            ) if show_cancel_button else None
        ]),
        dbc.CardBody([
            # Progress bar
            ...
            html.Div([
                ...
                html.Div(
                    ...
                    children=[
                        html.P("Operation: Initializing", className="mb-1"),
            ...
            ], className="text-center") if show_cancel_button else None
        ])
    ], id=card_id, style={"display": "none"})


def create_progress_list(
    list_id: str,
    title: str = "Active Operations",
    max_items: int = 5
) -> dbc.Card:
    """
    ...
    return dbc.Card([
        ...
        dbc.CardBody([
            html.Div(
                id=f"{list_id}-items",
                children=[
                    html.P("No active operations", className="text-muted text-center")
                ]
            )
        ])
    ], id=list_id)
    ...
    if include_interval:
        components.append(
            create_progress_interval(
                interval_id=f"{base_id}-interval",
                interval_ms=500
            )
        )
