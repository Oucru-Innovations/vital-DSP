...
def display_page(pathname: str) -> html.Div:
    ...
    logger.info(f"Current URL pathname: {pathname}")
    ...
    try:
        if pathname == "/" or pathname is None:
            ...
            logger.info(f"Upload layout type: {type(layout)}")
            return layout
        elif pathname == "/time-domain":
            logger.info("Returning time domain layout")
            layout = time_domain_layout()
            logger.info(f"Time domain layout type: {type(layout)}")
            return layout
...
def test_routing(pathname):
    """Test function to verify routing is working."""
    logger.info(f"Test routing callback triggered with pathname: {pathname}")
    return no_update
...
def register_page_routing_callbacks(app):
    ...
    # Add a simple test callback to verify routing is working
    @app.callback(
        Output("page-content", "style"),
        [Input("url", "pathname")]
    )
    def test_routing_callback(pathname):
        """Callback wrapper for test_routing function."""
        return test_routing(pathname)
