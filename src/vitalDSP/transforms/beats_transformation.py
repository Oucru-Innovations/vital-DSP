...
class RRTransformation:
    ...
    def compute_rr_intervals(self, options=None):
        ...
        )  # Minimum distance between peaks (e.g., 600 ms for ECG)
        # peaks, _ = find_peaks(self.transformed_signal, height=height, distance=distance)
    ...
    def impute_rr_intervals(self, intervals, method="adaptive", **kwargs):
        ...
        **kwargs
        ...
        if method == "adaptive":
            ...
            if nan_ratio < 0.05:
                # If less than 5% missing values, use linear interpolation
                return linear_interpolation(intervals)
            elif nan_ratio < 0.2:
                # If missing values are moderate, use spline interpolation
                order = kwargs.get("order", 3)
                return spline_interpolation(intervals, order)
            else:
                # For large gaps, use rolling mean
