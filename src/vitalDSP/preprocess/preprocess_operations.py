...
def respiratory_filtering(signal, sampling_rate, lowcut=0.1, highcut=0.5, order=4):
    ...
    if highcut >= sampling_rate / 2:
        raise ValueError(f"highcut ({highcut}) must be less than Nyquist frequency ({sampling_rate/2})")
    ...
    try:
        ...
        if np.any(np.isnan(filtered_signal)) or np.any(np.isinf(filtered_signal)):
            raise ValueError("Filter produced NaN or Inf values - filter may be unstable")
        
        if np.max(np.abs(filtered_signal)) > 1e6:
            # Filter is unstable, try with lower order
            if order > 1:
                return respiratory_filtering(signal, sampling_rate, lowcut, highcut, order-1)
            else:
