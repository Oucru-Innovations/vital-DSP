...
class SignalValidator:
    ...
    def validate_signal_pair(
        ...
        if require_same_length and len(sig1) != len(sig2):
            raise ValueError(f"{signal1_name} length {len(sig1)} != {signal2_name} length {len(sig2)}")
    ...
    def validate_frequency_parameters(
        ...
        if isinstance(cutoff, (list, tuple, np.ndarray)):
            ...
            if filter_type == "bandpass" and len(cutoff) != 2:
                raise ValueError("Bandpass filter requires exactly 2 cutoff frequencies")
            if filter_type == "bandstop" and len(cutoff) != 2:
                raise ValueError("Bandstop filter requires exactly 2 cutoff frequencies")
        else:
            if cutoff <= 0:
    ...
    def validate_window_parameters(
        ...
        if window_size > signal_length:
            raise ValueError(f"Window size {window_size} > signal length {signal_length}")
    ...
    def validate_nn_intervals(nn_intervals: Union[np.ndarray, list]) -> np.ndarray:
        ...
        if np.any(nn_intervals > 3000):  # > 3 seconds is unusual
            warnings.warn("Some NN intervals exceed 3000ms, which may indicate measurement errors")
        
        if np.any(nn_intervals < 300):  # < 0.3 seconds is unusual
            warnings.warn("Some NN intervals are less than 300ms, which may indicate measurement errors")
    ...
    def validate_threshold(threshold: float, threshold_type: str = "absolute") -> float:
        ...
        if threshold_type == "absolute" and threshold < 0:
            raise ValueError(f"Absolute threshold must be non-negative, got {threshold}")
        
        if threshold_type == "relative" and (threshold < 0 or threshold > 1):
            raise ValueError(f"Relative threshold must be between 0 and 1, got {threshold}")
...
def validate_signal_length(signal: Union[np.ndarray, list], min_length: int = 1) -> np.ndarray:
