...
class AdaptiveParameterAdjuster:
    ...
    def _classify_signal_type(self, signal: np.ndarray, fs: float, 
                            dominant_freq: float, peak_count: int) -> str:
    ...
    def adjust_filter_parameters(self, base_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'window_size' in adjusted_params:
            ...
            if char.length < 1000:
                ...
                adjusted_params['window_size'] = min(char.length // 10, base_window_size * 2)
            elif char.noise_level == 'low':
    ...
    def adjust_analysis_parameters(self, base_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'window_size' in adjusted_params:
            ...
            if char.length < 1000:
                ...
                adjusted_params['window_size'] = min(base_window_size * 2, char.length // 5)
            
            # Adjust based on signal type
            if char.signal_type == 'ecg':
                # ECG needs longer windows for reliable analysis
                adjusted_params['window_size'] = max(adjusted_params['window_size'], 1000)
            elif char.signal_type == 'respiratory':
                # Respiratory signals need very long windows
                adjusted_params['window_size'] = max(adjusted_params['window_size'], 2000)
        ...
        if 'step_size' in adjusted_params:
            ...
            if char.length < 1000:
                ...
                adjusted_params['step_size'] = min(base_step_size * 2, adjusted_params.get('window_size', 100) // 2)
    ...
    def adjust_feature_extraction_parameters(self, base_params: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if 'threshold' in adjusted_params and 'recurrence' in str(base_params.get('method', '')):
            base_threshold = adjusted_params['threshold']
    ...
    def adjust_respiratory_parameters(self, base_params: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if 'min_breath_duration' in adjusted_params:
            ...
            if char.dominant_frequency > 0:
                ...
                adjusted_params['min_breath_duration'] = max(0.5, suggested_min_duration)
        ...
        if 'max_breath_duration' in adjusted_params:
            ...
            if char.dominant_frequency > 0:
                ...
                adjusted_params['max_breath_duration'] = min(10.0, suggested_max_duration)
        ...
        if 'distance' in adjusted_params:
            ...
            if char.signal_type == 'respiratory':
                expected_period = 1.0 / char.dominant_frequency if char.dominant_frequency > 0 else 4.0
                suggested_distance = int(expected_period * char.sampling_rate * 0.3)
                adjusted_params['distance'] = max(10, suggested_distance)
        
        return adjusted_params
    
    def get_optimal_parameters(self, operation_type: str, base_params: Dict[str, Any]) -> Dict[str, Any]:
        """
...
def analyze_signal_characteristics(signal: np.ndarray, fs: float = 1.0) -> SignalCharacteristics:
    """
...
def get_optimal_parameters(operation_type: str, base_params: Dict[str, Any]) -> Dict[str, Any]:
    """
...
def optimize_filtering_parameters(signal: np.ndarray, fs: float, base_params: Dict[str, Any]) -> Dict[str, Any]:
...
def optimize_analysis_parameters(signal: np.ndarray, fs: float, base_params: Dict[str, Any]) -> Dict[str, Any]:
    """Optimize analysis parameters for a signal."""
    _global_adjuster.analyze_signal(signal, fs)
    return _global_adjuster.adjust_analysis_parameters(base_params)


def optimize_feature_extraction_parameters(signal: np.ndarray, fs: float, base_params: Dict[str, Any]) -> Dict[str, Any]:
...
def optimize_respiratory_parameters(signal: np.ndarray, fs: float, base_params: Dict[str, Any]) -> Dict[str, Any]:
