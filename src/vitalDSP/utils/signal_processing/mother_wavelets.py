...
class Wavelet:
    ...
    def db(order=4):
        ...
        _DB_LO = {
            ...
            3: np.array([
                0.3326705529500825,  0.8068915093110926,
            ...
            5: np.array([
                0.1601023979741929,  0.6038292697974185,
                0.7243085284377729,  0.1384281459013207,
               -0.2422948870663824, -0.0322448695846381,
                0.0775714938400459, -0.0062414902127983,
               -0.0125807519990820,  0.0033357252854738,
            ]),
            6: np.array([
                0.1115407433501095,  0.4946238903984530,
                0.7511339080210954,  0.3152503517092432,
               -0.2262646939654100, -0.1297668675672624,
                0.0975016055873224,  0.0275228655303053,
               -0.0315820810014865,  0.0005538422009938,
                0.0047772575109455, -0.0010773010853085,
            ]),
            7: np.array([
                0.0778520540850037,  0.3965393194819173,
                0.7291320908462368,  0.4697822874052152,
               -0.1439060039285212, -0.2240361849938500,
                0.0713092192668312,  0.0806126091510659,
               -0.0380299369350122, -0.0165745416306664,
                0.0125509985560993,  0.0004295779729214,
               -0.0018016407039998,  0.0003537137999745,
            ]),
            8: np.array([
                0.0544158422431049,  0.3128715909144659,
                0.6756307362973195,  0.5853546836548691,
               -0.0158291052563823, -0.2840155429615469,
                0.0004724845739124,  0.1287474266204837,
               -0.0173693010018083, -0.0440882539307971,
        ...
        _SYM_LO = {
            ...
            3: np.array([
                ...
                -0.1350110200102546,  0.4598775021184915,
                 0.8068915093110926,  0.3326705529500825,
            ]),
            4: np.array([
                -0.0757657147893406, -0.0296355276459541,
                 0.4976186676324578,  0.8037387518052163,
                 0.2978577956052774, -0.0992195435769354,
                -0.0126039672622612,  0.0322231006040427,
            ]),
            5: np.array([
                 0.0273330683451645, -0.0295194909260734,
                -0.2474985700611133,  0.4937216264309682,
                 0.7973510013999680,  0.3486048809346920,
                -0.0676328290612938, -0.1400842430439987,
                 0.0186477833781657,  0.0355423381571987,
            ]),
            6: np.array([
                 0.0154041093272182,  0.0034907120843304,
                -0.1179901111484105, -0.0483117425859981,
                 0.4910559419276396,  0.7876411410287941,
                 0.3379294217278806, -0.0726375227866000,
                -0.1663176508429673,  0.0046623726483920,
                 0.0121764802753706, -0.0039466905989270,
            ]),
            7: np.array([
                 0.0202767858330571, -0.0170857564509860,
                -0.1641586756690713,  0.0621656457376411,
                 0.4625362936739696,  0.7565821109183484,
                 0.3773551352142397, -0.0322431756784820,
                -0.2088830398028700,  0.0321067045087432,
                 0.0454282907382478, -0.0155049631071818,
                -0.0051526807186808,  0.0022025450523861,
            ]),
            8: np.array([
                -0.0033824159510061,  0.0005421323316990,
                 0.0316950878103452, -0.0076074873249766,
                -0.1432942383510542,  0.0612733590679088,
                 0.4813596512592012,  0.7771857517005235,
                 0.3644418948359564, -0.0519458381078751,
                -0.2272180351088840,  0.0363585669159919,
                 0.0510097726033448, -0.0210602925126954,
                -0.0053068792350380,  0.0033357252854738,
            ]),
        }
    ...
    def meyer(N=6):
        ...
        wavelet = (
            (4.0 / (3.0 * np.pi)) * np.cos(2 * np.pi * t) * np.sinc(4 * t / 3)
            + (8.0 / (3.0 * np.pi)) * np.sin(2 * np.pi * t) * np.sinc(8 * t / 3)
        )
    ...
    def biorthogonal(N=6, p=2, q=2):
        ...
        def bspline(x, order):
            ...
            with np.errstate(invalid="ignore", divide="ignore"):
                val = np.where(np.abs(x) < 1e-10, 1.0, (np.sin(np.pi * x / order) / (np.pi * x / order)) ** order)
            return val
