...
from filtering.advanced_filtering import AdvancedFiltering
...
def test_kalman_filter(sample_signal):
    filter = AdvancedFiltering(sample_signal)
...
def test_optimization_based_filter(sample_signal):
    filter = AdvancedFiltering(sample_signal)
    filtered_signal = filter.optimization_based_filter(loss_function='mse')
    assert len(filtered_signal) == len(sample_signal), "Filtered signal length mismatch"
...
def test_gradient_descent_filter(sample_signal):
    filter = AdvancedFiltering(sample_signal)
    filtered_signal = filter.gradient_descent_filter(learning_rate=0.01)
    assert len(filtered_signal) == len(sample_signal), "Filtered signal length mismatch"
...
def test_ensemble_filtering(sample_signal):
    filter = AdvancedFiltering(sample_signal)
    filtered_signal = filter.ensemble_filtering(filters=[SignalFiltering.moving_average, SignalFiltering.gaussian])
    assert len(filtered_signal) == len(sample_signal), "Filtered signal length mismatch"
...
def test_convolution_based_filter(sample_signal):
    ...
    filter = AdvancedFiltering(sample_signal)
    filtered_signal = filter.convolution_based_filter(kernel=kernel)
    assert len(filtered_signal) == len(sample_signal), "Filtered signal length mismatch"
