import pytest
import numpy as np
from vitalDSP.advanced_computation.reinforcement_learning_filter import (
...
def test_q_learning(rl_filter):
    ...
    assert rl_filter.q_table.shape == (
        len(rl_filter.signal),
        len(rl_filter.action_space),
    )
...
def test_dqn_training(rl_filter):
    ...
    """
    rl_filter.train_dqn(
        episodes=10, batch_size=4, gamma=0.99, epsilon=0.1, target_update=2
    )
    assert rl_filter.q_table.shape == (
        len(rl_filter.signal),
        len(rl_filter.action_space),
...
def test_apply_action(rl_filter):
    ...
    if rl_filter.action_space[action] != 0:
        ...
        )  # If action is zero, signal should remain unchanged
...
def test_policy_network(rl_filter):
    ...
    assert action in range(
        len(rl_filter.action_space)
    )  # Ensure a valid action is sampled
...
def test_collect_trajectories(rl_filter):
    ...
    policy_network = rl_filter._initialize_policy_network()
    value_network = rl_filter._initialize_value_network()
    trajectories = rl_filter._collect_trajectories(
        policy_network, value_network, gamma=0.99, lambda_=0.95
    )
...
def test_calculate_advantage(rl_filter):
    ...
    advantage = rl_filter._calculate_advantage(
        state, action, reward, next_state, value_network, gamma=0.99, lambda_=0.95
    )
