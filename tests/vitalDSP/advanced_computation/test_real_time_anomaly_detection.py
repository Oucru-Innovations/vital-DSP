...
def test_statistical_detection_z_score(anomaly_detector):
    ...
    data_stream = [
        ...
        1,
        10,
    ]  # Last point is a noticeable outlier with lower threshold
    anomalies = []
    for point in data_stream:
        anomaly = anomaly_detector.detect_statistical(
            point, method="z_score", threshold=1.0
        )
...
def test_statistical_detection_moving_average(anomaly_detector):
    ...
    for point in data_stream:
        anomaly = anomaly_detector.detect_statistical(
            point, method="moving_average", threshold=1.0
        )
        anomalies.append(anomaly)
...
def test_svm_training_and_detection(anomaly_detector):
    ...
    anomaly = anomaly_detector.detect_svm(
        10
    )  # Detecting an outlier in standard normal data
...
def test_autoencoder_detection_no_training(anomaly_detector):
    with pytest.raises(ValueError):
        anomaly_detector.detect_autoencoder(
            np.random.randn(3)
        )  # Error if autoencoder not trained
...
def test_wavelet_detection(anomaly_detector):
    ...
    data_stream = np.concatenate(
        [np.sin(np.linspace(0, 10, 5)), [50]]
    )  # Last point is a large outlier
    anomalies = []
    for point in data_stream:
        anomaly = anomaly_detector.detect_wavelet(
            point, wavelet_name="haar", level=1, threshold=10.0
        )
        anomalies.append(anomaly)
    assert (
        anomalies.count(True) == 1
    )  # Only the last point should be detected as an anomaly
...
def test_update_model_svm(anomaly_detector):
    ...
    assert hasattr(
        anomaly_detector.models["svm"], "training_data"
    ), "SVM model should have 'training_data' attribute"
    assert (
        6 in anomaly_detector.models["svm"].training_data
    ), "New data point should be in the SVM training data"
...
def test_update_model_autoencoder(anomaly_detector):
    ...
    assert (
        autoencoder_model.training_data[-1] == new_data_point
    ), "The new data point should be appended."
