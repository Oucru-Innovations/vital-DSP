...
def test_sparse_representation_fft(sparse_processing, test_signal):
    ...
    assert np.allclose(sparse_rep, np.fft.fft(test_signal))  # Ensure it matches FFT output
...
def test_thresholding(sparse_processing, test_signal):
    ...
    assert np.all(np.abs(thresholded_sparse[np.abs(sparse_rep) < 0.1]) == 0)  # Ensure thresholding worked
...
def test_thresholding_all_above_threshold(sparse_processing):
    ...
    thresholded_sparse = sparse_processing.thresholding(sparse_rep, threshold=0.1)
    assert np.array_equal(thresholded_sparse, sparse_rep)  # No values should be set to zero
...
def test_reconstruction_fft(sparse_processing, test_signal):
    ...
    assert np.allclose(reconstructed_signal, np.fft.ifft(thresholded_sparse))  # Check reconstruction accuracy
...
def test_reconstruction_custom_basis(sparse_processing, test_signal):
    ...
    reconstructed_signal = sparse_processing.reconstruction(sparse_rep, custom_inverse_basis)
    assert isinstance(reconstructed_signal, np.ndarray)
    assert np.array_equal(reconstructed_signal, test_signal)  # Ensure the reconstruction matches the original signal
