import pytest
import numpy as np
from vitalDSP.advanced_computation.generative_signal_synthesis import (
...
def test_generate_random_noise(signal_synthesizer):
    ...
    signal = signal_synthesizer.generate(
        method="random_noise", length=100, mean=0, std_dev=1
    )
...
def test_generate_gaussian_process(signal_synthesizer):
    ...
    signal = signal_synthesizer.generate(
        method="gaussian_process", length=100, mean=0, std_dev=1, correlation=0.5
    )
...
def test_generate_autoregressive(signal_synthesizer):
    ...
    coeffs = [0.9, -0.5]
    signal = signal_synthesizer.generate(
        method="autoregressive", length=100, coeffs=coeffs
    )
    assert len(signal) == 100
    assert isinstance(signal, np.ndarray)
...
def test_generate_custom_function(signal_synthesizer):
    ...
    signal = signal_synthesizer.generate(
        method="custom_function", length=100, func=func
    )
    assert len(signal) == 100
    assert isinstance(signal, np.ndarray)
