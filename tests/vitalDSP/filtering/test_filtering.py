...
def test_signal_lowpass_filter():
    ...
    bp_filter = BandpassFilter(band_type="butter", fs=100)
    cutoff = 0.3
    filtered_signal = bp_filter.signal_lowpass_filter(
        signal_data, cutoff=cutoff, order=4
    )
...
def test_signal_highpass_filter():
    ...
    bp_filter = BandpassFilter(band_type="butter", fs=100)
    cutoff = 0.3
    filtered_signal = bp_filter.signal_highpass_filter(
        signal_data, cutoff=cutoff, order=4
    )
    assert len(filtered_signal) == len(signal_data)
    assert np.all(np.isfinite(filtered_signal))
...
def test_signal_highpass_filter_short_signal():
    ...
    cutoff = 0.3
    with pytest.raises(
        ValueError, match="The length of the input vector x must be greater than"
    ):
...
def test_moving_average(sample_signal):
    ...
    assert len(filtered_signal) == (
        len(sample_signal)
    ), "Filtered signal length mismatch"
...
def test_gaussian(sample_signal):
    ...
    assert np.all(
        np.abs(filtered_signal - sample_signal) < 1
    ), "Gaussian filtering failed"
...
def test_savgol_filter(sample_signal):
    filtered_signal = SignalFiltering.savgol_filter(
        sample_signal, window_length=5, polyorder=2
    )
    assert len(filtered_signal) == len(sample_signal), "Filtered signal length mismatch"
    assert np.all(
        np.abs(filtered_signal - sample_signal) < 1
    ), "Savitzky-Golay filtering failed"
    with pytest.raises(
        ValueError, match="window_length must be a positive odd integer"
    ):
        SignalFiltering.savgol_filter(sample_signal, window_length=4, polyorder=2)
    with pytest.raises(
        ValueError, match="window_length is too small for the polynomial order"
    ):
...
def test_optimization_based_filter(sample_signal):
    ...
    assert np.all(
        np.abs(filtered_signal - sample_signal) < 1
    ), "Optimization-based filtering failed"
...
def test_gradient_descent_filter(sample_signal):
    ...
    target = np.array([1, 1.5, 2.5, 3, 2.5, 2, 2.5, 3.5, 3, 2.5])
    filter = AdvancedSignalFiltering(sample_signal)
    filtered_signal = filter.gradient_descent_filter(
        target, learning_rate=0.01, iterations=20
    )
    assert len(filtered_signal) == len(sample_signal), "Filtered signal length mismatch"
...
def test_convolution_based_filter(sample_signal):
    ...
    assert np.all(
        np.abs(filtered_signal - sample_signal) < 1
    ), "Convolution-based filtering failed"


def test_attention_based_filter(sample_signal):
    filter = AdvancedSignalFiltering(sample_signal)
    filtered_signal = filter.attention_based_filter(
