...
def test_get_flat_timestamp():
    ...
    assert isinstance(
        flat_list[0], dt.datetime
    )  # Ensure the timestamps are of type datetime
...
def test_safe_get_flat_ecg_timestamp():
    ...
    shifted_series = pd.Series(
        {"timestamp": "2024-01-01 12:00:02+0000"}
    )  # 2 seconds later
    ...
    mock_chunk = pd.DataFrame(
        {
            ...
            "timestamp": [
                ...
                "2024-01-01 12:00:02+0000",
            ],  # 2 seconds apart
        }
    )
    mock_read_csv.return_value = [mock_chunk]

    pleth_col, date_col = process_in_chunks(
        "dummy_path", chunk_size=2, fs=3, data_type="ecg"
    )

    assert pleth_col == [1, 2, 3, 4, 5, 6]
    assert (
        len(date_col) == 9
    )  # 6 samples from each chunk, accounting for duration between timestamps
