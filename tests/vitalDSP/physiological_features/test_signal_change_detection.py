import numpy as np
import pytest
from vitalDSP.physiological_features.signal_change_detection import (
    SignalChangeDetection,
)
...
def test_absolute_difference(sample_signal):
    ...
    np.testing.assert_array_equal(
        abs_diff, expected_diff, "Absolute difference calculation failed"
    )
...
def test_variance_based_detection(sample_signal):
    ...
    np.testing.assert_almost_equal(
        ...
        expected_variances,
        decimal=5,
        err_msg="Variance-based detection failed",
    )
...
def test_energy_based_detection(sample_signal):
    ...
    np.testing.assert_array_equal(
        energies, expected_energies, "Energy-based detection failed"
    )
...
def test_adaptive_threshold_detection(sample_signal):
    ...
    np.testing.assert_array_equal(
        changes, expected_changes, "Adaptive threshold detection failed"
    )
...
def test_ml_based_change_detection_no_model(sample_signal):
    ...
    np.testing.assert_array_equal(
        changes, expected_changes, "ML-based change detection failed"
    )
...
def test_ml_based_change_detection_with_custom_model(sample_signal):
    ...
    np.testing.assert_array_equal(
        changes, expected_changes, "Custom ML-based detection failed"
    )
