...
def test_compute_weighted_moving_average(sample_signal):
    ...
    expected = np.array(
        [1.9, 2.9, 3.9, 4.9, 5.9]
    )  # Adjusted values based on correct weighted averages
    assert len(weighted_avg) == len(
        expected
    ), "Expected length of weighted_avg to be equal to length of expected"
...
def test_compute_exponential_smoothing(sample_signal):
    ...
    expected = np.array([1.0, 1.3, 1.81, 2.467, 3.227, 4.059, 4.941])
...
def test_compute_polynomial_trend(sample_signal_long):
    ...
    expected = np.array(
        [1.25, 2.21428571, 3.75, 5.85714286, 8.53571429, 11.78571429, 15.60714286]
    )
    assert len(polynomial_trend) == len(
        expected
    ), "Expected length of weighted_avg to be equal to length of expected"
