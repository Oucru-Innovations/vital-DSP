...
from vitalDSP.visualization.plot_estimate_rr import (
    plot_rr_estimations,
...
def setup_stubs(monkeypatch):
    ...
    monkeypatch.setattr(
        "vitalDSP.respiratory_analysis.estimate_rr.fft_based_rr", fft_based_rr_stub
    )
    monkeypatch.setattr(
        "vitalDSP.respiratory_analysis.estimate_rr.peak_detection_rr",
        peak_detection_rr_stub,
    )
    monkeypatch.setattr(
        "vitalDSP.respiratory_analysis.estimate_rr.time_domain_rr", time_domain_rr_stub
    )
    monkeypatch.setattr(
...
def test_plot_rr_estimations(setup_stubs):
    ...
    fig = plot_rr_estimations(
        signal, sampling_rate, preprocess="bandpass", lowcut=0.1, highcut=0.5
    )
