...
from vitalDSP.filtering.signal_filtering import (
    SignalFiltering,
)  # Assuming the filtering functions are implemented
from vitalDSP.visualization.filtering_visualization import (
    FilteringVisualization,
)  # Assuming this is the location of the class
import pytest
...
from vitalDSP.visualization.transform_visualization import (
    SignalDecompositionVisualization,
    DCTWaveletFusionVisualization,
    STFTVisualization,
    WaveletFFTfusionVisualization,
    MFCCVisualization,
    ChromaSTFTVisualization,
    ERPVisualization,
    TFRVisualization,
)
...
def test_dct_wavelet_fusion(mock_show):
    ...
    assert isinstance(
        viz.fusion, DCTWaveletFusion
    )  # Check DCTWaveletFusion object creation
...
def test_wavelet_fft_fusion(mock_show):
    ...
    viz.compare_original_fusion()
    assert isinstance(
        viz.fusion, WaveletFFTfusion
    )  # Check WaveletFFTfusion object creation
...
def test_chroma_stft_visualization(mock_show):
    ...
    assert isinstance(
        chroma_stft_viz.chroma_stft, ChromaSTFT
    )  # Check ChromaSTFT object creation
...
def test_time_freq_representation(mock_show):
    ...
    assert isinstance(
        tfr_viz.tfr, TimeFreqRepresentation
    )  # Check TimeFreqRepresentation object creation
