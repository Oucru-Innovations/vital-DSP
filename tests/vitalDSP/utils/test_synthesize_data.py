import pytest
import numpy as np
from vitalDSP.utils.synthesize_data import (generate_sinusoidal, generate_square_wave, generate_noisy_signal, 
                         generate_ecg_signal, generate_resp_signal, generate_synthetic_ppg, 
...
def test_generate_square_wave():
    ...
    assert isinstance(signal, np.ndarray)
    assert len(signal) == int(sampling_rate * duration)
    assert np.all(np.unique(signal) == np.array([-1.0, 1.0]))  # Square wave with values -1 and 1
...
def test_generate_ecg_signal():
    ...
    Anoise = 0.01
    hrmean = 70
    ecg_signal, ipeaks = generate_ecg_signal(sfecg=sfecg, N=N, Anoise=Anoise, hrmean=hrmean)
...
def test_generate_synthetic_ppg():
    ...
    sampling_rate = 1000
    heart_rate = 60
    ppg_signal = generate_synthetic_ppg(duration=duration, sampling_rate=sampling_rate, heart_rate=heart_rate)
...
def test_generate_synthetic_ppg_reversed():
    ...
    sampling_rate = 1000
    heart_rate = 60
    ppg_signal = generate_synthetic_ppg_reversed(duration=duration, sampling_rate=sampling_rate, heart_rate=heart_rate)
