...
def test_remove_invalid_rr_intervals(rr_transformation):
    ...
    rr_intervals = np.array([0.2, 0.9, 1.2, 1.5, 2.5])  # Simulated RR intervals with three invalid
    filtered_rr_intervals = rr_transformation.remove_invalid_rr_intervals(rr_intervals)
    assert np.isnan(filtered_rr_intervals).sum() == 3, "Three invalid intervals should be NaN"
...
def test_remove_invalid_rr_intervals_sudden_change(rr_transformation):
    ...
    rr_intervals = np.array([0.8, 0.82, 0.85, 1.2, 0.9])  # Sudden increase between two intervals
    filtered_rr_intervals = rr_transformation.remove_invalid_rr_intervals(rr_intervals, sudden_change_threshold=0.2)
    assert np.isnan(filtered_rr_intervals).sum() > 0, "Sudden changes should be marked as invalid"
...
def test_impute_rr_intervals_linear(rr_transformation):
    ...
    imputed_intervals = rr_transformation.impute_rr_intervals(rr_intervals, method='linear')
    assert not np.isnan(imputed_intervals).any(), "All NaNs should be imputed with linear interpolation"

def test_impute_rr_intervals_adaptive(rr_transformation):
    # Test adaptive imputation
    rr_intervals = np.array([0.8, np.nan, 0.82, np.nan, np.nan, 0.85, 0.83])
    imputed_intervals = rr_transformation.impute_rr_intervals(rr_intervals, method='adaptive')
    assert not np.isnan(imputed_intervals).any(), "All NaNs should be imputed adaptively"
...
def test_process_rr_intervals(rr_transformation):
    ...
    assert isinstance(processed_rr_intervals, np.ndarray), "Processed RR intervals should be an array"
    assert not np.isnan(processed_rr_intervals).any(), "All invalid RR intervals should be imputed"
...
def test_reconsider_trends(rr_transformation):
    ...
    assert np.isnan(rr_intervals_with_trends).sum() == 2, "Trend reconsideration should only handle gradual trends"
...
def test_invalid_signal_length():
    ...
    try:
        ...
        pytest.fail("Signal length should be long enough and peaks should be detected for RR interval computation.")
