...
def mock_wavelet_transform(mocker):
    ...
    mock_wavelet_instance.perform_wavelet_transform.return_value = np.array(
        [[7, 8, 9], [10, 11, 12]]
    )
    mocker.patch(
        "vitalDSP.transforms.wavelet_transform.WaveletTransform",
        return_value=mock_wavelet_instance,
    )
...
def test_tfr_invalid_method(sample_signal):
    ...
    with pytest.raises(
        ValueError, match="Unsupported method. Use 'stft' or 'wavelet'."
    ):
        tfr.compute_tfr()
