...
def test_apply_artifact_removal(transformer, mocker):
    ...
    mocker.patch('vitalDSP.filtering.artifact_removal.ArtifactRemoval.baseline_correction', return_value=transformer.signal)
    ...
    mocker.patch('vitalDSP.filtering.artifact_removal.ArtifactRemoval.mean_subtraction', return_value=transformer.signal)
    transformer.apply_artifact_removal(method="mean_subtraction")
    assert transformer.signal is not None

    # Test median filter removal
    mocker.patch('vitalDSP.filtering.artifact_removal.ArtifactRemoval.median_filter_removal', return_value=transformer.signal)
    transformer.apply_artifact_removal(method="median_filter_removal", options={"kernel_size": 3})
    assert transformer.signal is not None

    # Test wavelet denoising
    mocker.patch('vitalDSP.filtering.artifact_removal.ArtifactRemoval.wavelet_denoising', return_value=transformer.signal)
    transformer.apply_artifact_removal(method="wavelet_denoising", options={"wavelet_type": "db", "level": 1, "order": 4})
    assert transformer.signal is not None

    # Test adaptive filtering with reference signal
    reference_signal = np.random.randn(1000)
    mocker.patch('vitalDSP.filtering.artifact_removal.ArtifactRemoval.adaptive_filtering', return_value=transformer.signal)
    transformer.apply_artifact_removal(method="adaptive_filtering", options={"reference_signal": reference_signal})
    assert transformer.signal is not None

    # Test PCA artifact removal
    mocker.patch('vitalDSP.filtering.artifact_removal.ArtifactRemoval.pca_artifact_removal', return_value=transformer.signal)
    transformer.apply_artifact_removal(method="pca_artifact_removal", options={"num_components": 1})
    assert transformer.signal is not None

    # Test ICA artifact removal
    mocker.patch('vitalDSP.filtering.artifact_removal.ArtifactRemoval.ica_artifact_removal', return_value=transformer.signal)
    transformer.apply_artifact_removal(method="ica_artifact_removal", options={"num_components": 1, "max_iterations": 1000})
    assert transformer.signal is not None
...
def test_apply_bandpass_filter(transformer, mocker):
    ...
    mocker.patch('vitalDSP.filtering.signal_filtering.BandpassFilter.signal_highpass_filter', return_value=transformer.signal)
    mocker.patch('vitalDSP.filtering.signal_filtering.BandpassFilter.signal_lowpass_filter', return_value=transformer.signal)
...
def test_apply_smoothing(transformer, mocker):
    ...
    mocker.patch('vitalDSP.filtering.signal_filtering.SignalFiltering.moving_average', return_value=transformer.signal)
    mocker.patch('vitalDSP.filtering.signal_filtering.SignalFiltering.gaussian', return_value=transformer.signal)
...
def test_apply_transformations(transformer, mocker):
    ...
    transformed_signal = transformer.apply_transformations(options=options, method_order=method_order)
