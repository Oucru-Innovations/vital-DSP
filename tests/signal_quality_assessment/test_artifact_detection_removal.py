...
def test_threshold_artifact_detection(example_signal):
    ...
    assert np.array_equal(artifacts, [2, 5]), f"Expected artifacts at indices [2, 5], got {artifacts}"
...
def test_moving_average_artifact_removal():
    ...
    expected = np.array([1.33333333, 2., 35., 36., 37., 6., 6.66666667])
...
def test_wavelet_artifact_removal():
    ...
    assert len(cleaned_signal) <= len(expected), "Expected cleaned signal to be less than or equal in terms of length"
...
def test_median_filter_artifact_removal():
    ...
    expected = np.array([1., 2., 3., 5., 6., 6., 7.])
...
def test_iterative_artifact_removal():
    ...
    expected = np.array([1., 2., 3., 5., 5., 6., 7.])
