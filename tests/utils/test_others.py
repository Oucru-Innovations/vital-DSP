...
def test_min_max_normalization():
    ...
    expected_output_custom = np.array([-1., -0.5, 0., 0.5, 1.])
    
    np.testing.assert_almost_equal(normalized_signal_custom, expected_output_custom, decimal=6)

    # Test with a single value (edge case)
    signal_single = np.array([5])
    normalized_single = min_max_normalization(signal_single)
    expected_single = np.array([0.])  # Min-Max normalization of a single value will return 0 in default range
