...
def test_handle_file_upload():
    ...
    status, figure, data = handle_file_upload(file_content, filename)  # Expecting 3 return values now

    # Check if the status is correct
    assert status == "File test.csv uploaded successfully.", "File upload status is incorrect."
