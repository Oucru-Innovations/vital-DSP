...
def test_upload_section():
    ...
    """
    assert type(upload_layout) == html.Div, "UploadSection layout is not rendering correctly."
    ...
    assert type(children[0]) == (dcc.Store), "First child should be a dcc.Store Component."
    ...
    assert type(card_body_1) == (dbc.CardBody), "First card should contain dbc.CardBody."
    ...
    assert type(upload_component) == (dcc.Upload), "First card should contain dcc.Upload component."
    ...
    assert type(card_body_2) == (dbc.CardBody), "Second card should contain dbc.CardBody."
    ...
    assert type(graph_component) == (dcc.Graph), "Second card should contain dcc.Graph for data visualization."
