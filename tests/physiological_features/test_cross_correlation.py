...
def sample_signals():
    ...
    signal2 = np.sin(np.linspace(0, 10, 500)) + np.random.normal(0, 0.1, 500)  # Mock PPG signal
...
def test_compute_cross_correlation(cross_corr_features):
    ...
    expected_length = len(cross_corr_features.signal1) + len(cross_corr_features.signal2) - 1
    assert len(cross_corr) == expected_length, f"Expected {expected_length}, got {len(cross_corr)}"
    
    # Lag should be within the expected range
    assert lag >= -(len(cross_corr_features.signal1) - 1) and lag <= (len(cross_corr_features.signal1) - 1)
...
def test_compute_normalized_cross_correlation(cross_corr_features):
    ...
    assert np.all(norm_corr >= -1.0) and np.all(norm_corr <= 1.0), \
