...
def test_z_score_anomaly_detection(anomaly_detector):
    ...
    assert np.all(np.abs((anomaly_detector.signal - np.mean(anomaly_detector.signal)) / np.std(anomaly_detector.signal))[anomalies] > 1.0)
...
def test_moving_average_anomaly_detection(anomaly_detector):
    ...
    anomalies = anomaly_detector.detect_anomalies(method="moving_average", window_size=5, threshold=0.2)
    assert isinstance(anomalies, np.ndarray)
    assert anomalies.shape[0] > 0  # Ensure anomalies are detected
